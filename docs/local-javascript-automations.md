...
- `cerebrumEducationCompiler.js`: revision-driven generation after education saves/startup and visible compilation status.
...
Existing semantic schedules, vision requests, learning and autonomous reasoning remain in place. New scheduled semantic tasks can keep their schedule in a real `.js` file and delegate only the work requiring the model, such as fresh weather summaries with TTS. KNX sensor addresses in the instruction are preserved; local readings must not be confused with forecasts or stale data. They are not automatically converted, and this feature alone does not eliminate every background LLM call. Runtime replay tooling, source history restoration controls, a syntax-highlighting editor, arbitrary plugin writers and asynchronous guest APIs are not implemented.
