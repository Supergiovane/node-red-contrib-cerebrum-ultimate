...
Ask Cerebrum in chat what to automate. For deterministic reminders, schedules and event rules, it can create a real `.js` function and keep it running locally. Saving **AI Education** automatically starts generation of missing functions. Saved instructions are also checked after startup. The page displays generation progress or the reason a function could not be created; **Check AI Education** retries explicitly. The Web sidebar’s **JavaScript automations** page displays these actual functions, their purpose, status and last run. It starts empty; no example files or programming task are presented to the user.
