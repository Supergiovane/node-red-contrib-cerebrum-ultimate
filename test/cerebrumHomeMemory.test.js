...
const {
  ...
  buildCerebrumProactiveFallback,
...
describe('Cerebrum bounded home intelligence memory', () => {
  ...
  it('recognizes cover and room labels in every supported language', () => {
    ...
    cases.forEach(([label, kind, area]) => {
      const semantic = inferCerebrumHomeSemantic({
        label,
        dpt: '5.001',
        role: 'status'
      })
  ...
  it('keeps the Markdown file below the configured hard limit without mixing in configured Education', () => {
    let memory = createEmptyCerebrumHomeMemory()
    for (let index = 0; index < 500; index++) {
      memory = addBoundedCerebrumObservation(memory, {
      ...
      memory = addBoundedCerebrumNotification(memory, {
    ...
    expect(rendered.markdown).to.include('AI Education — configured on the Cerebrum node')
    expect(rendered.markdown).to.include('not stored in learned memory')
    const restored = parseCerebrumHomeMemoryMarkdown(rendered.markdown)
  ...
  it('does not let multibyte Education consume the learned-memory byte budget', () => {
    ...
    const rendered = buildCerebrumHomeMemoryMarkdown({
      memory: createEmptyCerebrumHomeMemory(),
      education,
      maxKb: 64
    })
  ...
  it('learns only aggregate cover duration habits with bounded samples', () => {
    let memory = createEmptyCerebrumHomeMemory()
    memory = updateCerebrumCoverHabit(memory, {
      ga: '1/2/3',
  ...
  it('recognizes reliable open states but ignores command objects', () => {
    ...
    expect(classifyCerebrumOpenState({ semantic, dpt: '5.001', payload: 0 })).to.include({ open: true })
    expect(classifyCerebrumOpenState({ semantic, dpt: '5.001', payload: 100 })).to.include({ open: false })

    const binary = classifyCerebrumOpenState({
    ...
    expect(classifyCerebrumOpenState({
  ...
  it('provides proactive fallback text in every supported language', () => {
    ...
    languages.forEach(language => {
      const message = buildCerebrumProactiveFallback({
        language,
  ...
  it('learns repeated temporal state patterns but predicts only after occupant confirmation', () => {
    ...
    ;[3, 4, 5, 6, 7, 10].forEach(day => {
      memory = updateCerebrumTemporalHabit(memory, {
        source: 'knx',
        objectId: '1/1/1',
        label: 'Kitchen light',
        area: 'kitchen',
    ...
    expect(findCerebrumHabitCandidates(memory)).to.deep.equal([])
    ...
    expect(findCerebrumHabitCandidates(memory)).to.have.length(1)
    expect(findCerebrumHabitPredictions(memory, {
      date: new Date(2026, 7, 11, 7, 30),
      windowMinutes: 20
    })).to.deep.equal([])

    memory = applyCerebrumHabitDecision(memory, {
      habitId: habit.id,
      operation: 'modify',
  ...
  it('renders an Italian read-only explanation without exposing JSON structure', async () => {
    const { formatCerebrumMemorySimpleText } = await import('../ui/cerebrumUltimate-vue/src/cerebrumMemoryView.mjs')
    const memory = {
  ...
  it('extracts editable JSON and preserves the backup envelope', async () => {
    ...
    } = await import('../ui/cerebrumUltimate-vue/src/cerebrumMemoryView.mjs')
    const original = '<!-- CEREBRUM_HOME_MEMORY_V1\n{"version":2,"habits":[]}\nCEREBRUM_HOME_MEMORY_END -->\n\n# Readable backup\n'
  ...
  it('renders the native Cerebrum Learning file as localized read-only text', async () => {
    const {
      formatChatLearningSimpleText,
      parseChatLearningNativeFile
    } = await import('../ui/cerebrumUltimate-vue/src/chatLearningView.mjs')
    const nativeFile = [
      '# Cerebrum native chat-learning context',
      'CEREBRUM_CHAT_CONTEXT\t3',
    ...
    expect(text).to.include('Apprendimento Cerebrum — vista semplificata')
    expect(text).to.include('Vista di sola lettura')
    ...
    expect(text).not.to.include('CEREBRUM_CHAT_CONTEXT')
    expect(text).not.to.include('\t')
  })

  it('keeps the simplified Cerebrum Learning view read-only and reports malformed native files', async () => {
    const { parseChatLearningNativeFile } = await import('../ui/cerebrumUltimate-vue/src/chatLearningView.mjs')
    expect(() => parseChatLearningNativeFile('CEREBRUM_CHAT_CONTEXT\t3\nCREATED_AT\tnow\nUPDATED_AT\tnow\nSESSION\tbroken\tnow\n'))
      .to.throw('SESSION without END_SESSION')

    const fs = require('fs')
    const path = require('path')
    const webUi = fs.readFileSync(path.join(__dirname, '..', 'ui', 'cerebrumUltimate-vue', 'src', 'App.vue'), 'utf8')
    expect(webUi).to.match(/chatLearningViewMode:\s*\["native",\s*"simple"\]\.includes/)
    expect(webUi).to.include('state.chatLearningViewMode === \'native\'')
    expect(webUi).to.include('Simplified Cerebrum Learning, read-only')
    expect(webUi).to.include('state.chatLearningViewMode !== \'native\'')
    expect(webUi).to.include('state.chatLearningViewMode === \'simple\'')
    expect(webUi).to.include('? chatLearningSimpleView.value')
    const navigation = fs.readFileSync(path.join(__dirname, '..', 'ui', 'cerebrumUltimate-vue', 'src', 'components', 'CerebrumNavigation.vue'), 'utf8')
    expect(navigation).to.include('aria-controls="knx-sidebar-submenu"')
    expect(navigation).to.include('class="nav-submenu"')
    expect(webUi).to.include('<h2>KNX Areas</h2>')
    expect(webUi).to.match(/isViewingTestResultOnly\s*\?\s*["']KNX Test Result["']\s*:\s*["']KNX Tests["']/)
    expect(webUi).to.include('<h2>KNX Test Results</h2>')
    expect(webUi).to.include('<h2>Node-RED Flow Builder <span class="beta-badge">BETA</span></h2>')
    expect(webUi).to.include('class="more-actions workspace-maintenance"')
    expect(webUi).to.include('<h4>Choose area and template</h4>')
    expect(webUi).to.include('<h3>Select test plan</h3>')
    expect(webUi).to.include('<h4>Save or run</h4>')
    expect(webUi).to.include('state.showAiPlanner = true')
    expect(webUi).to.include('<h3>Create a KNX area first</h3>')
    expect(webUi).to.include('class="workflow-assistant-callout"')
    expect(webUi).to.include('class="workflow-section workflow-prerequisite"')
    expect(webUi).not.to.include(':disabled="!suggestedAreas.length" @click="activateSidebarTab(\'tests\')"')
  })
})
