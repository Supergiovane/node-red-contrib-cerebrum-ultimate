...
} = require('../nodes/utils/cerebrumScheduler')
...
describe('Cerebrum semantic scheduler', () => {
  it('passes the resolved Markdown filename through the atomic writer filePath contract', () => {
    ...
    expect(source).to.include('writeAtomicUtf8File({ filePath: markdownPath, content: buildCerebrumScheduleMarkdown(node._scheduleStore) })')
    expect(source).not.to.include('writeAtomicUtf8File({ markdownPath, content: buildCerebrumScheduleMarkdown(node._scheduleStore) })')
  })
  ...
  it('creates, lists and cancels schedules only for the owning chat', () => {
    ...
    let outcome = applyCerebrumScheduleActions({
      store: makeStore([], now),
    ...
    outcome = applyCerebrumScheduleActions({
      store: outcome.store,
    ...
    const listedForA = applyCerebrumScheduleActions({
    ...
    const foreignCancellation = applyCerebrumScheduleActions({
      store: storeWithBothOwners,
    ...
    expect(listActiveCerebrumSchedules(foreignCancellation.store, { now })).to.have.length(2)

    const ownCancellation = applyCerebrumScheduleActions({
    ...
    expect(listActiveCerebrumSchedules(ownCancellation.store, { sessionId: 'chat-a', now })).to.deep.equal([])
    expect(listActiveCerebrumSchedules(ownCancellation.store, { sessionId: 'chat-b', now }).map(task => task.id))
  ...
  it('stores a recurring five-day monitor from semantic action data without inspecting its wording', () => {
    ...
    const outcome = applyCerebrumScheduleActions({
      store: makeStore([], now),
      actions: [{
        operation: 'create',
        kind: 'monitor',
  ...
  it('rounds and clamps recurring intervals while preserving zero as one-time execution', () => {
    ...
    expect(normalizeIntervalMinutes(1)).to.equal(CEREBRUM_SCHEDULE_MIN_INTERVAL_MINUTES)
    expect(normalizeIntervalMinutes(12.6)).to.equal(13)
    expect(normalizeIntervalMinutes(CEREBRUM_SCHEDULE_MAX_INTERVAL_MINUTES + 1000))
      .to.equal(CEREBRUM_SCHEDULE_MAX_INTERVAL_MINUTES)

    const normalized = normalizeCerebrumScheduleActions([{
      operation: 'create',
      kind: 'reminder',
    ...
    expect(normalized.accepted[0].intervalMinutes).to.equal(CEREBRUM_SCHEDULE_MIN_INTERVAL_MINUTES)
  })
  ...
  it('claims an overdue recurrence once and advances directly beyond now without a catch-up burst', () => {
    ...
    const firstClaim = claimDueCerebrumSchedules({ store: initialStore, now, limit: 8 })
    ...
    const duplicateClaim = claimDueCerebrumSchedules({ store: firstClaim.store, now, limit: 8 })
  ...
  it('retries an interrupted one-time reminder but never replays a claimed one-time command', () => {
    ...
    const reminderClaim = claimDueCerebrumSchedules({ store: initialStore, now, limit: 1 })
    ...
    expect(reminderClaim.store.tasks.find(task => task.id === 'schedule-reminder-once')).to.include({
      status: 'active',
      lastStatus: 'running',
      nextRunAt: new Date(now + CEREBRUM_SCHEDULE_ONE_TIME_LEASE_MS).toISOString()
    })
    const reminderOnlyStore = makeStore(
      reminderClaim.store.tasks.filter(task => task.id === 'schedule-reminder-once'),
      now
    )
    expect(claimDueCerebrumSchedules({ store: reminderOnlyStore, now: now + 15000 }).claimed).to.deep.equal([])

    const reminderCompletion = completeCerebrumScheduleRun({
    ...
    const commandClaim = claimDueCerebrumSchedules({ store: reminderCompletion.store, now, limit: 1 })
  ...
  it('expires elapsed tasks and recovers interrupted persisted runs safely on restart', () => {
    ...
    const normalized = normalizeCerebrumScheduleStore(makeStore([
      makeTask({
  ...
  it('allows cancellation while the claimed final recurrence is running and ignores its late completion', () => {
    ...
    const claimed = claimDueCerebrumSchedules({ store: initialStore, now: runAt })

    expect(claimed.claimed[0]).to.include({ status: 'expired', lastStatus: 'running' })
    const cancelled = applyCerebrumScheduleActions({
      store: claimed.store,
    ...
    const lateCompletion = completeCerebrumScheduleRun({
  ...
  it('records completion errors and bounded notification fingerprints', () => {
    const runAt = Date.parse('2026-08-29T10:00:00.000Z')
    const claimed = claimDueCerebrumSchedules({
    ...
    const completed = completeCerebrumScheduleRun({
      store: claimed.store,
      taskId: 'schedule-test',
      ok: false,
  ...
  it('persists the original human request and renders it in the generated Markdown', () => {
    ...
    const markdown = buildCerebrumScheduleMarkdown(outcome.store, { now })
    expect(markdown).to.include('# Cerebrum Plans and Reminders')
