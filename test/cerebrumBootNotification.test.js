...
const { getCerebrumBootFallbackCopy } = require('../nodes/cerebrumUltimate').__test

describe('Cerebrum startup assistant notification', () => {
  it('provides a Cerebrum fallback in every supported language', () => {
    const languages = ['en', 'it', 'de', 'fr', 'es', 'zh-CN']
    languages.forEach(language => {
      const message = getCerebrumBootFallbackCopy({ language })
  ...
  it('keeps fallback diagnostics short and on one line', () => {
    const message = getCerebrumBootFallbackCopy({
      language: 'it',
