...
} = require('./utils/cerebrumTelegramVoice')
const {
...
const {
  ...
  CEREBRUM_SCHEDULE_MAX_INSTRUCTION_CHARS,
  applyCerebrumScheduleActions,
  buildCerebrumScheduleMarkdown,
  buildCerebrumSchedulePromptContext,
  claimDueCerebrumSchedules,
  completeCerebrumScheduleRun,
  createEmptyCerebrumScheduleStore,
  listActiveCerebrumSchedules,
  normalizeCerebrumScheduleActions,
  normalizeCerebrumScheduleStore
} = require('./utils/cerebrumScheduler')
...
const resolveCerebrumLlmTimeoutMs = ({ configuredTimeoutMs } = {}) => {
  const configured = Number(configuredTimeoutMs)
  const fallback = CEREBRUM_LLM_TIMEOUT_MIN_MS
  const requested = Number.isFinite(configured) && configured > 0 ? Math.round(configured) : fallback
  return Math.max(CEREBRUM_LLM_TIMEOUT_MIN_MS, requested)
}

const normalizeCerebrumReasoningEffort = (value) => {
  const normalized = String(value || '').trim().toLowerCase()
  return CEREBRUM_REASONING_EFFORT_OPTIONS.includes(normalized) ? normalized : 'default'
}

const resolveCerebrumReasoningRequestFields = ({ provider, effort } = {}) => {
  const normalizedProvider = normalizeCerebrumLlmProvider(provider)
  const normalizedEffort = normalizeCerebrumReasoningEffort(effort)
...
const normalizeCerebrumLocalContextTokens = (value) => {
  const requested = Math.round(Number(value) || 0)
...
const resolveCerebrumLocalGenerationBudget = ({ provider, contextTokens, configuredMaxTokens, reasoningEffort, workload = 'conversation' } = {}) => {
  const normalizedProvider = String(provider || '').trim().toLowerCase()
  ...
  const effort = normalizeCerebrumReasoningEffort(reasoningEffort)
...
const measureCerebrumPromptContext = ({ body, provider, model } = {}) => {
...
const buildCerebrumHomeMemoryRevision = (memory) => {
  const normalized = normalizeCerebrumHomeMemory(memory)
  normalized.updatedAt = ''
  if (normalized.reconciler) normalized.reconciler.lastTickAt = ''
  return crypto.createHash('sha256').update(JSON.stringify(normalized), 'utf8').digest('hex')
}

const resolveCerebrumCameraProviderConfigNodeId = provider => String(provider && (
  provider.configNodeId ||
...
const summarizeDetectedCerebrumCameraAdapters = ({ registry, node } = {}) => {
  const sourceRegistry = registry || getCerebrumCameraAdapterRegistry()
  const adapters = new Map(sourceRegistry && sourceRegistry.adapters instanceof Map ? sourceRegistry.adapters : [])
  const detectedProviders = new Map(sourceRegistry && sourceRegistry.providers instanceof Map ? sourceRegistry.providers : [])
  const usedProviders = new Map()
  ...
  if (node && node._cameraProviders instanceof Map) {
    node._cameraProviders.forEach((provider, id) => usedProviders.set(String(id), provider))
  } else {
    detectedProviders.forEach((provider, providerId) => {
      if (isCerebrumCameraProviderSelected({ provider, providerId, node })) usedProviders.set(String(providerId), provider)
    })
  }
  ...
  return Array.from(adapters.entries()).map(([adapterId, value]) => {
    ...
    const detectedMatchingProviders = []
    const usedProviderIds = new Set()
    detectedProviders.forEach((provider, providerId) => {
      if (String(provider && provider.adapterId || '') !== adapterId) return
      detectedMatchingProviders.push({
        id: String(providerId),
        title: String(provider && (provider.controllerName || provider.title || provider.id) || providerId),
        configNodeId: resolveCerebrumCameraProviderConfigNodeId(provider),
        selected: usedProviders.get(String(providerId)) === provider
      })
    })
    usedProviders.forEach((provider, providerId) => {
      if (String(provider && provider.adapterId || '') === adapterId) usedProviderIds.add(String(providerId))
    })
    let cameraCount = 0
    cameras.forEach(camera => {
      const cameraAdapterId = String(camera && camera.adapterId || '')
      const cameraProviderId = String(camera && camera.providerId || '')
      if (cameraAdapterId === adapterId || usedProviderIds.has(cameraProviderId)) cameraCount += 1
    })
    return {
      id: adapterId,
      ...
      providerCount: detectedMatchingProviders.length,
      usedProviderCount: usedProviderIds.size,
      cameraCount,
      providers: detectedMatchingProviders.sort((left, right) => left.title.localeCompare(right.title))
    }
...
const buildCerebrumCompatibleNodeSummary = ({
  ...
  if (knxConfigTypeAvailable === true || knxConfigId) {
    entries.push({
      id: 'knx-ultimate',
      title: 'KNX Ultimate',
      packageName: 'node-red-contrib-knx-ultimate',
      kind: 'config',
      configNodeType: 'knxUltimate-config',
  ...
  const unifiAdapter = adapters.find(adapter => String(adapter && adapter.id || '') === 'unifi-ultimate')
  if (unifiConfigTypeAvailable === true || unifiConfigId || unifiAdapter) {
    entries.push({
      id: 'unifi-ultimate',
      title: String(unifiAdapter && unifiAdapter.title || 'UniFi Ultimate / Protect'),
      packageName: String(unifiAdapter && unifiAdapter.packageName || 'node-red-contrib-unifi-ultimate'),
      kind: 'config',
      configNodeType: 'unifi-protect-config',
      configProperty: 'unifiProtectConfig',
      configNodeId: unifiConfigId,
      detected: true,
      configured: !!unifiConfigId,
      usedInChat: Number(unifiAdapter && unifiAdapter.usedProviderCount) > 0,
      providerCount: Math.max(0, Number(unifiAdapter && unifiAdapter.providerCount) || 0),
      usedProviderCount: Math.max(0, Number(unifiAdapter && unifiAdapter.usedProviderCount) || 0),
      cameraCount: Math.max(0, Number(unifiAdapter && unifiAdapter.cameraCount) || 0)
    })
  }

  adapters
    .filter(adapter => adapter && adapter.id && String(adapter.id) !== 'unifi-ultimate')
    .forEach(adapter => {
      entries.push({
        id: String(adapter.id),
        title: String(adapter.title || adapter.id),
        packageName: String(adapter.packageName || ''),
        kind: 'camera',
        detected: true,
        configured: Number(adapter.providerCount) > 0,
        usedInChat: Number(adapter.usedProviderCount) > 0,
        providerCount: Math.max(0, Number(adapter.providerCount) || 0),
        usedProviderCount: Math.max(0, Number(adapter.usedProviderCount) || 0),
        cameraCount: Math.max(0, Number(adapter.cameraCount) || 0)
      })
    })
  ...
  return entries.sort((left, right) => {
    ...
    const rightConfig = right.kind === 'config' ? 0 : 1
    return leftConfig - rightConfig || left.title.localeCompare(right.title)
  })
}

const buildCerebrumTtsUltimateAnnouncementMessage = ({
  text,
  reason = '',
  sourceNodeId = '',
  sessionId = ''
...
const summarizeCerebrumChatContext = ({ node, nodeId, redUserDir } = {}) => {
  const rawNodeId = String((node && node.id) || nodeId || '').trim()
  const safeNodeId = rawNodeId.replace(/[^A-Za-z0-9_.-]/g, '_').slice(0, 160)
  const configuredBaseDir = path.join(String(redUserDir || process.cwd()), 'cerebrumultimatestorage')
  const baseDir = path.resolve(configuredBaseDir)
  const cerebrumDir = path.join(baseDir, 'cerebrum')
  const memoryDir = path.join(cerebrumDir, 'memory')
  const schedulesDir = path.join(cerebrumDir, 'schedules')
  const configDir = path.join(cerebrumDir, 'config')
  const debugDir = path.join(cerebrumDir, 'debug')
  const telegramArchiveRoot = path.join(cerebrumDir, 'history')
  const telegramNodeDir = safeNodeId ? path.join(telegramArchiveRoot, safeNodeId) : ''
  const adapterArchiveRoot = path.join(cerebrumDir, 'adapter-history')
  const adapterNodeDir = safeNodeId ? path.join(adapterArchiveRoot, safeNodeId) : ''

  const files = [
    {
      id: 'chatContext',
      name: 'cerebrum-chat-context.knxctx',
      path: path.join(memoryDir, 'cerebrum-chat-context.knxctx')
    },
    {
      id: 'homeMemory',
      name: 'cerebrum-home-memory.md',
      path: path.join(memoryDir, 'cerebrum-home-memory.md')
    }
  ]
  if (safeNodeId) {
    files.push({
      ...
      path: path.join(schedulesDir, `cerebrum-schedules-${safeNodeId}.json`)
    })
    files.push({
      id: 'schedulesReadable',
      name: `cerebrum-schedules-${safeNodeId}.md`,
      path: path.join(schedulesDir, `cerebrum-schedules-${safeNodeId}.md`)
    })
    files.push({
      id: 'assistantConfig',
      name: `cerebrum-config-${safeNodeId}.json`,
      path: path.join(configDir, `cerebrum-config-${safeNodeId}.json`)
    })
    files.push({
      id: 'lastChatPrompt',
      name: `cerebrum-last-chat-prompt-${safeNodeId}.txt`,
      path: path.join(debugDir, `cerebrum-last-chat-prompt-${safeNodeId}.txt`)
    })
  }
...
const buildCerebrumFirstRunExperience = ({ catalog, areasSnapshot, language = 'en', displayName = '', assistantEnabled = false } = {}) => {
  ...
  const totals = {
    ...
    logicalFunctionsEstimate: estimateCerebrumLogicalFunctions(list),
...
const isCerebrumSafeFirstRunPrompt = (question) => {
...
const bindSharedCerebrumState = ({ registry, filePath, node, property, initialValue }) => {
...
const releaseSharedCerebrumState = ({ registry, filePath, node }) => {
...
const CEREBRUM_FLOW_CORE_NODES = [
...
const cerebrumScanObjectEntries = (body) => {
  const entries = {}
...
const cerebrumSliceBalanced = (text, openIndex) => {
...
const cerebrumMatchAfter = (text, regex) => {
...
const cerebrumParseDefaultsFields = (defaultsBody) => {
  const fields = {}
  const entries = cerebrumScanObjectEntries(defaultsBody)
  Object.keys(entries).forEach((key) => {
    const inner = entries[key] || ''
    const configType = cerebrumMatchAfter(inner, /\btype\s*:\s*['"]([^'"]+)['"]/)
    fields[key] = configType ? { configType, isConfig: true } : {}
  })
  return fields
}

let cerebrumPackageNodeCatalogCache = null
const CEREBRUM_OPTIONAL_NODE_RED_PACKAGES = Object.freeze([
  'node-red-contrib-knx-ultimate',
  'node-red-contrib-home-assistant-websocket'
])
const CEREBRUM_EXCLUDED_LEGACY_NODE_TYPES = new Set(['knxUltimateAI', 'knxUltimateAIHomeAssistant'])

const resolveCerebrumNodeRedPackageSources = () => {
  const packagePaths = [path.join(__dirname, '..', 'package.json')]
  CEREBRUM_OPTIONAL_NODE_RED_PACKAGES.forEach(packageName => {
    try { packagePaths.push(require.resolve(`${packageName}/package.json`)) } catch (error) { /* optional package is not installed */ }
  })
  return Array.from(new Set(packagePaths.map(packagePath => path.resolve(packagePath)))).map(packagePath => {
    try {
      return { packagePath, packageRoot: path.dirname(packagePath), manifest: require(packagePath) }
    } catch (error) {
      return null
    }
  }).filter(Boolean)
...
// Read every registerType(...) declaration in Cerebrum and its installed,
// optional automation packages. This lets Flow Builder discover KNX Ultimate
// and Home Assistant without making either one a runtime dependency.
// and return a catalog: [{ type, paletteLabel, category, inputs, outputs, fields }].
const buildCerebrumPackageNodeCatalog = () => {
  if (cerebrumPackageNodeCatalogCache) return cerebrumPackageNodeCatalogCache
  const catalog = []
  const seen = new Set()
  resolveCerebrumNodeRedPackageSources().forEach(source => {
    const nodeMap = (source.manifest['node-red'] && source.manifest['node-red'].nodes) || {}
    Object.keys(nodeMap).forEach((mapKey) => {
      try {
        const jsRel = String(nodeMap[mapKey] || '').replace(/^[/\\]+/, '')
        const jsPath = path.resolve(source.packageRoot, jsRel)
        const htmlPath = jsPath.replace(/\.js$/i, '.html')
        ...
        while ((m = re.exec(html))) {
          const type = m[1]
          if (seen.has(type) || CEREBRUM_EXCLUDED_LEGACY_NODE_TYPES.has(type)) continue
          seen.add(type)
          const objOpen = html.indexOf('{', m.index + m[0].length - 1)
          if (objOpen < 0) continue
          const objText = cerebrumSliceBalanced(html, objOpen)
          const category = cerebrumMatchAfter(
            objText,
            /\bcategory\s*:\s*['"]([^'"]+)['"]/
          )
          const paletteLabel = cerebrumMatchAfter(
            objText,
            /\bpaletteLabel\s*:\s*['"]([^'"]+)['"]/
          )
          const inputsRaw = cerebrumMatchAfter(objText, /\binputs\s*:\s*(\d+)/)
          const outputsRaw = cerebrumMatchAfter(objText, /\boutputs\s*:\s*(\d+)/)
          let fields = {}
          ...
          if (defIdx >= 0) {
            ...
            const defaultsBlock = cerebrumSliceBalanced(objText, braceIdx)
            fields = cerebrumParseDefaultsFields(defaultsBlock.slice(1, -1))
          }
          catalog.push({
            type,
            packageName: String(source.manifest.name || ''),
        ...
        // A malformed optional package must never prevent Cerebrum from loading.
      }
    })
  })
  cerebrumPackageNodeCatalogCache = catalog
  return catalog
}

// Combined catalog (package + core), the set of config-node types, and a
// per-type map of which fields are config references.
const buildCerebrumFlowCatalog = () => {
  const packageNodes = buildCerebrumPackageNodeCatalog()
  const all = packageNodes.concat(CEREBRUM_FLOW_CORE_NODES)
...
const renderCerebrumCatalogForPrompt = (catalog) => {
  const lines = []
  catalog.nodes
...
const parseCerebrumFlowFromLlm = (content) => {
...
const normalizeCerebrumGeneratedFlow = ({ rawNodes, catalog, knxServerId, existingConfigByType, genId }) => {
  const warnings = []
  ...
  const tabNode = { id: tabId, type: 'tab', label: 'Cerebrum generated flow', disabled: false, info: '' }
  return { nodes: [tabNode].concat(out), warnings, tabId }
}

const sendCerebrumVueIndex = (req, res) => {
  const entryPath = path.join(cerebrumVueDistDir, 'index.html')
  fs.readFile(entryPath, 'utf8', (error, html) => {
    if (error || typeof html !== 'string') {
      res.status(503).type('text/plain').send('Cerebrum Vue build not found. Run "npm run cerebrum:build" in the module root.')
      return
    }
...
const CEREBRUM_UNSUPPORTED_STRUCTURED_SCHEMA_KEYS = new Set([
  'minLength',
  'maxLength',
  'pattern',
  'format',
...
const sanitizeCerebrumStructuredOutputSchema = (value) => {
  if (Array.isArray(value)) return value.map(sanitizeCerebrumStructuredOutputSchema)
  if (!value || typeof value !== 'object') return value
  return Object.fromEntries(Object.entries(value)
    .filter(([key]) => !CEREBRUM_UNSUPPORTED_STRUCTURED_SCHEMA_KEYS.has(key))
    .map(([key, candidate]) => [key, sanitizeCerebrumStructuredOutputSchema(candidate)]))
}
...
const normalizeCerebrumDptId = (value) => String(value || '').trim().replace(/^dpt\s*/i, '')
...
const normalizeCerebrumSpeechActionCandidate = (value) => {
  const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {}
...
const normalizeCerebrumMemoryActions = (value) => {
  const accepted = []
  const rejected = []
...
const collectCerebrumWebSources = (results, maxSources = CEREBRUM_WEB_MAX_SOURCES) => {
  const collected = []
  const seen = new Set()
  const append = (candidate, fallback = {}) => {
    ...
    collected.push({
      ...
      title: sanitizeCerebrumWebSourceText(source.title || fallback.title || url, 200),
      url,
      retrievedAt: sanitizeCerebrumWebSourceText(source.retrievedAt || fallback.retrievedAt || '', 64)
    })
  }
  ...
  return collected.slice(0, Math.max(1, Number(maxSources) || CEREBRUM_WEB_MAX_SOURCES))
}

const buildCerebrumWebResearchContext = ({ results, maxChars = 24000 } = {}) => {
  const source = Array.isArray(results) ? results : []
  if (!source.length) return ''
  const sources = collectCerebrumWebSources(source, CEREBRUM_WEB_MAX_SOURCES)
  ...
  source.forEach((result, resultIndex) => {
    const operation = sanitizeCerebrumWebSourceText(result && result.operation || 'unknown', 24)
    if (!result || result.ok !== true) {
      lines.push(`Result ${resultIndex + 1} (${operation}) failed: ${sanitizeCerebrumWebSourceText(result && result.error || 'unknown error', 500)}`)
      return
    }
    if (Array.isArray(result.results)) {
      lines.push(`Search query: ${sanitizeCerebrumWebSourceText(result.query, 500)}`)
      result.results.slice(0, CEREBRUM_WEB_MAX_SOURCES).forEach(item => {
        const url = String(item && item.url || '').trim()
        const sourceId = idByUrl.get(url) || `R${resultIndex + 1}`
        lines.push(`[${sourceId}] ${sanitizeCerebrumWebSourceText(item && item.title || url, 240)}`)
        lines.push(`URL: ${url}`)
        const snippet = sanitizeCerebrumWebSourceText(item && (item.text || item.snippet) || '', 1400)
    ...
    const sourceId = idByUrl.get(url) || `R${resultIndex + 1}`
    lines.push(`[${sourceId}] ${sanitizeCerebrumWebSourceText(result.title || url, 240)}`)
    lines.push(`URL: ${url}`)
    if (result.retrievedAt) lines.push(`Retrieved: ${sanitizeCerebrumWebSourceText(result.retrievedAt, 64)}`)
...
const appendCerebrumWebSources = ({ content, sources, language = 'en' } = {}) => {
  const text = String(content || '').trim()
  const list = (Array.isArray(sources) ? sources : []).slice(0, CEREBRUM_WEB_MAX_SOURCES)
  ...
  const lines = list.map(source => {
    const sourceId = sanitizeCerebrumWebSourceText(source && source.id, 16)
    const retrievedAt = sanitizeCerebrumWebSourceText(source && source.retrievedAt, 64)
    return `- ${sourceId ? `[${sourceId}] ` : ''}${sanitizeCerebrumWebSourceText(source && source.title || source && source.url, 180)} — ${String(source && source.url || '').trim()}${retrievedAt ? ` — ${retrievedLabel}: ${retrievedAt}` : ''}`
  })
  return `${text}\n\n${label}:\n${lines.join('\n')}`
}

const buildCerebrumWebResearchFingerprint = (results) => {
...
const extractCerebrumQuestion = (msg) => {
...
const resolveCerebrumSessionId = (msg) => {
  const source = msg && typeof msg === 'object' ? msg : {}
  const candidates = [
    source.cerebrum && source.cerebrum.sessionId,
    source.sessionId,
    source.chatId,
...
const classifyCerebrumConfirmation = ({ msg, question, topic } = {}) => {
  const source = msg && typeof msg === 'object' ? msg : {}
  if (source.cerebrum && source.cerebrum.confirm === true) return 'confirm'
  if (source.cerebrum && source.cerebrum.confirm === false) return 'cancel'
...
const resolveCerebrumLanguage = (msg, fallback = 'en', question = '', responseLanguage = '') => {
  const source = msg && typeof msg === 'object' ? msg : {}
  const explicitCandidates = [
    source.cerebrum && source.cerebrum.language,
    source.language,
    source.payload && typeof source.payload === 'object' ? source.payload.language : ''
  ]
  const inferredCandidates = [
    responseLanguage,
    detectCerebrumLanguageFromText(question),
...
const getCerebrumConfirmationCopy = (language) => {
  const copies = {
    en: {
...
const getCerebrumThinkingCopy = (language) => {
  const copies = {
    en: 'I’m thinking…',
    it: 'Sto pensando…',
    de: 'Ich denke nach…',
...
const getCerebrumRequestStatusLabel = (language) => {
  const labels = {
    en: 'Request',
    it: 'Richiesta',
    de: 'Anfrage',
...
const getCerebrumReadCopy = (language) => {
  const copies = {
    en: {
...
const formatCerebrumReadResults = ({ operations, results, language }) => {
  const reads = Array.isArray(operations) ? operations : []
  const settled = Array.isArray(results) ? results : []
  const copy = getCerebrumReadCopy(language)
  const values = []
  const missing = []
...
const buildCerebrumReadResultMetadata = ({ operations, results } = {}) => {
  const reads = Array.isArray(operations) ? operations : []
  const settled = Array.isArray(results) ? results : []
...
const buildCerebrumRoutineInspectionContext = ({ routine, readResults } = {}) => {
  const descriptor = normalizeCerebrumRoutineDescriptor(routine)
...
const formatCerebrumRoutineExecutionReport = ({ routine, commands, results, language } = {}) => {
  const descriptor = normalizeCerebrumRoutineDescriptor(routine)
  const operations = Array.isArray(commands) ? commands : []
  const settled = Array.isArray(results) ? results : []
  const copy = getCerebrumConfirmationCopy(language)
  const verified = []
  const unverified = []
...
const buildCerebrumConfirmationRequest = ({
  sessionId,
  expiresAt,
  commandCount,
  copy,
  ...
  const routineDescriptor = normalizeCerebrumRoutineDescriptor(routine)
  if (routineDescriptor.active) request.routine = routineDescriptor
  return request
}

const cloneCerebrumInputMessage = (inputMessage, cloneMessage, onError) => {
...
const safeCerebrumSend = ({ outputs, send, onError }) => {
  try {
    if (typeof send !== 'function') throw new Error('Cerebrum output sender is unavailable')
    send(outputs)
    return true
  } catch (error) {
    try {
...
const compileCerebrumChatAdapter = ({ code, direction = 'chat' } = {}) => {
  ...
  try {
    ...
    throw new Error(`Invalid Cerebrum ${adapterDirection} adapter: ${error.message || error}`)
  }
}

const executeCerebrumChatAdapter = ({
  adapter,
  msg,
  inputMessage,
  node,
  ...
  if (result && typeof result.then === 'function') {
    throw new Error(`Cerebrum ${adapter.direction || 'chat'} adapter must be synchronous`)
  }
  if (result === undefined || result === null) return null
  if (!result || typeof result !== 'object' || Array.isArray(result)) {
    throw new Error(`Cerebrum ${adapter.direction || 'chat'} adapter must return msg or no value`)
  }
  return result
}

const applyCerebrumChatMediaPresetFallback = ({ preset, message, inputMessage } = {}) => {
  if (String(preset || '') !== 'windkh-telegrambot' || !message || typeof message !== 'object') return message
  const image = message.cerebrum && message.cerebrum.image
...
const applyCerebrumRedBotOutputEnvelopeFallback = ({ preset, message, inputMessage } = {}) => {
...
const applyCerebrumChatConfirmationPresetFallback = ({ preset, message } = {}) => {
  if (String(preset || '') !== 'windkh-telegrambot' || !message || typeof message !== 'object') return message
  ...
  const suggestions = message.cerebrum && Array.isArray(message.cerebrum.suggestions)
    ? message.cerebrum.suggestions
  ...
  const type = String(message.cerebrum && message.cerebrum.type || '')
...
const validateCerebrumPayloadForDpt = ({ dpt, payload }) => {
...
const coerceCerebrumCommandPayload = (value, { dpt } = {}) => {
...
const resolveCerebrumOperationEvent = (candidate) => {
...
const normalizeCerebrumCommandCandidates = ({
  commands,
  catalog,
  maxCommands = 5,
  maxReadCommands = 20,
  ...
  sourceCommands.slice(0, totalLimit).forEach((candidate, index) => {
    try {
      const item = candidate && typeof candidate === 'object' ? candidate : {}
      const event = resolveCerebrumOperationEvent(item)
      ...
      const catalogDpt = normalizeCerebrumDptId(catalogItem.dpt)
      if (!catalogDpt) throw new Error('the ETS catalog has no DPT for this destination')
      const requestedDpt = normalizeCerebrumDptId(item.dpt)
      ...
      validateCerebrumPayloadForDpt({ dpt: catalogDpt, payload })
      acceptedWrites += 1
      accepted.push({
        destination,
        dpt: catalogDpt,
...
const normalizeCerebrumGaRoleActions = ({ actions, catalog } = {}) => {
  const safeCatalog = Array.isArray(catalog) ? catalog : []
...
const applyCerebrumGaRoleActionsToCatalog = ({ catalog, actions } = {}) => {
...
const normalizeCerebrumGaRoleExperience = (value) => {
  const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {}
...
const applyCerebrumCatalogAccessConfiguration = ({
  catalog,
...
const isLlmRequestTimeoutError = (error) => {
  ...
  return code === 'CEREBRUM_LLM_TIMEOUT' ||
...
const requestBufferedLlmHttp = async ({
  ...
  for (let redirectCount = 0; redirectCount <= maxRedirects; redirectCount++) {
    ...
    if (nextUrl.origin !== currentUrl.origin) {
      ...
      redirectError.code = 'CEREBRUM_LLM_CROSS_ORIGIN_REDIRECT'
      throw redirectError
    }
  ...
  try {
    ...
    try {
      ...
      if (isLlmRequestTimeoutError(error)) {
        ...
        timeoutError.code = 'CEREBRUM_LLM_TIMEOUT'
...
const decorateChatCompletionsModelError = ({ error, model, url }) => {
  ...
  const decorated = new Error(
    ...
    `Legacy /v1/completions models are not supported by Cerebrum.${originalMessage ? ` Provider response: ${originalMessage}` : ''}`
  )
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      if (promptCacheOptionsUnsupported && response && typeof response === 'object') {
        response._cerebrumPromptCacheOptionsUnsupported = true
      }
      return response
    } catch (error) {
      lastError = error
...
module.exports = function (RED) {
  ...
  if (!adminEndpointsRegistered) {
    ...
    RED.httpAdmin.get('/cerebrumUltimate/sidebar/page-vue', RED.auth.needsPermission('cerebrumUltimate.read'), (req, res) => {
      sendCerebrumVueIndex(req, res)
    })

    RED.httpAdmin.get('/cerebrumUltimate/sidebar/page/assets/:file', RED.auth.needsPermission('cerebrumUltimate.read'), (req, res) => {
      sendStaticFileSafe({
        rootDir: cerebrumVueDistDir,
    ...
    RED.httpAdmin.get('/cerebrumUltimate/sidebar/assets/:file', RED.auth.needsPermission('cerebrumUltimate.read'), (req, res) => {
      sendStaticFileSafe({
        rootDir: cerebrumVueDistDir,
        relativePath: path.join('assets', String(req.params?.file || '')),
        res
    ...
    RED.httpAdmin.get('/cerebrumUltimate/sidebar/page-vue/assets/:file', RED.auth.needsPermission('cerebrumUltimate.read'), (req, res) => {
      sendStaticFileSafe({
        rootDir: cerebrumVueDistDir,
        relativePath: path.join('assets', String(req.params?.file || '')),
        res
    ...
    RED.httpAdmin.get('/cerebrumUltimate/adapters', RED.auth.needsPermission('cerebrumUltimate.read'), async (req, res) => {
      try {
        ...
        res.json({
          ...
          chatContext: summarizeCerebrumChatContext({
            node: deployedNode,
            nodeId,
    ...
    RED.httpAdmin.get('/cerebrumUltimate/sidebar/nodes', RED.auth.needsPermission('cerebrumUltimate.read'), (req, res) => {
      try {
    ...
    RED.httpAdmin.get('/cerebrumUltimate/sidebar/state', RED.auth.needsPermission('cerebrumUltimate.read'), async (req, res) => {
      try {
        const nodeId = req.query?.nodeId ? String(req.query.nodeId) : ''
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/chat-learning/save', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        if (!nodeId) {
          res.status(400).json({ error: 'Missing nodeId' })
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/chat-learning/reset', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        if (!nodeId) {
          res.status(400).json({ error: 'Missing nodeId' })
    ...
    RED.httpAdmin.get('/cerebrumUltimate/sidebar/home-memory', RED.auth.needsPermission('cerebrumUltimate.read'), async (req, res) => {
      try {
        const nodeId = req.query?.nodeId ? String(req.query.nodeId) : ''
        if (!nodeId) {
          res.status(400).json({ error: 'Missing nodeId' })
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/home-memory/save', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        if (!nodeId) {
          res.status(400).json({ error: 'Missing nodeId' })
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/home-memory/reset', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        if (!nodeId) {
          res.status(400).json({ error: 'Missing nodeId' })
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/ask', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/areas/save', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/areas/reset', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        const areaId = req.body?.areaId ? String(req.body.areaId) : ''
        if (!nodeId) {
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/areas/delete', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        const areaId = req.body?.areaId ? String(req.body.areaId) : ''
        if (!nodeId) {
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/areas/delete-llm', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        if (!nodeId) {
          res.status(400).json({ error: 'Missing nodeId' })
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/areas/catalog', RED.auth.needsPermission('cerebrumUltimate.read'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        if (!nodeId) {
          res.status(400).json({ error: 'Missing nodeId' })
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/areas/ga-role/save', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        if (!nodeId) {
          res.status(400).json({ error: 'Missing nodeId' })
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/areas/create', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        if (!nodeId) {
          res.status(400).json({ error: 'Missing nodeId' })
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/areas/regenerate-llm', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        if (!nodeId) {
          res.status(400).json({ error: 'Missing nodeId' })
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/areas/suggest-llm', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        if (!nodeId) {
          res.status(400).json({ error: 'Missing nodeId' })
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/profiles/save', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        if (!nodeId) {
          res.status(400).json({ error: 'Missing nodeId' })
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/profiles/delete', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/profiles/run', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        const areaId = req.body?.areaId ? String(req.body.areaId) : ''
        const profileId = req.body?.profileId ? String(req.body.profileId) : ''
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/actuator-tests/delete', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/actuator-tests/run', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        if (!nodeId) {
          res.status(400).json({ error: 'Missing nodeId' })
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/test-plans/catalog', RED.auth.needsPermission('cerebrumUltimate.read'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        const areaId = req.body?.areaId ? String(req.body.areaId) : ''
        if (!nodeId) {
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/test-plans/generate', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        const areaId = req.body?.areaId ? String(req.body.areaId) : ''
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/flow/generate', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        const prompt = req.body?.prompt ? String(req.body.prompt) : ''
        const language = req.body?.language
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/test-plans/save', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        if (!nodeId) {
          res.status(400).json({ error: 'Missing nodeId' })
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/test-plans/delete', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/test-plans/run', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        if (!nodeId) {
          res.status(400).json({ error: 'Missing nodeId' })
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/test-plans/run-step', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        const areaId = req.body?.areaId ? String(req.body.areaId) : ''
        if (!nodeId) {
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/test-results/save', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
        if (!nodeId) {
          res.status(400).json({ error: 'Missing nodeId' })
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/test-results/delete', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const nodeId = req.body?.nodeId ? String(req.body.nodeId) : ''
    ...
    RED.httpAdmin.post('/cerebrumUltimate/sidebar/tts/googletranslate', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
    ...
    RED.httpAdmin.post('/cerebrumUltimate/lmstudio/select-model', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const body = req.body || {}
    ...
    RED.httpAdmin.post('/cerebrumUltimate/models', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const body = req.body || {}
        const nodeId = body.nodeId ? String(body.nodeId) : ''
    ...
    RED.httpAdmin.post('/cerebrumUltimate/ollama/pull', RED.auth.needsPermission('cerebrumUltimate.write'), async (req, res) => {
      try {
        const body = req.body || {}
        const nodeId = body.nodeId ? String(body.nodeId) : ''
        let baseUrl = body.baseUrl ? String(body.baseUrl) : ''
  ...
  function cerebrumUltimate (config) {
    RED.nodes.createNode(this, config)
    const node = this

    node.serverKNX = config.server ? (RED.nodes.getNode(config.server) || undefined) : undefined
    node.unifiProtectConfigId = String(config.unifiProtectConfig || '').trim()
    node.unifiProtectConfig = node.unifiProtectConfigId ? (RED.nodes.getNode(node.unifiProtectConfigId) || undefined) : undefined
    node.cerebrumStorageDir = path.join(String(RED.settings.userDir || process.cwd()), 'cerebrumultimatestorage')
    node.knxIntegrationAvailable = !!node.serverKNX && knxDptAvailable

    node.name = config.name || 'Cerebrum'
    ...
    node.llmProvider = normalizeCerebrumLlmProvider(config.llmProvider)
    ...
    node.webMaxCallsPerHour = normalizeCerebrumWebMaxCallsPerHour(config.webMaxCallsPerHour)
    ...
    const updateConversationStatus = ({ type, question = '', language = 'en' } = {}) => {
      if (type === 'thinking') {
        updateStatus({
          ...
          fill: 'blue',
          shape: 'ring',
          text: getCerebrumThinkingCopy(language)
        })
        return
      }
      ...
      updateStatus({
        ...
        fill: 'grey',
        shape: 'dot',
        text: preview ? `${getCerebrumRequestStatusLabel(language)}: ${preview}` : getCerebrumRequestStatusLabel(language)
      })
    }

    const compileConfiguredChatAdapter = ({ code, direction }) => {
      try {
        return compileCerebrumChatAdapter({ code, direction })
      } catch (error) {
        try { node.sysLogger?.error(`cerebrumUltimate ${direction} adapter compile error: ${error.message || error}`) } catch (e) { /* ignore */ }
    ...
    const buildSummary = (now) => {
      ...
      gaLabels.BUS = node.name || 'Cerebrum'
    ...
    const buildLLMPrompt = ({ question, summary, limits = {} } = {}) => {
      ...
      const adapterLines = recentAdapterEvents
        .map(formatCerebrumAdapterHistoryEventForPrompt)
        .filter(Boolean)
    ...
    const getGaCatalogSnapshot = () => {
      ...
      const authorizedCatalog = applyCerebrumCatalogAccessConfiguration({
        catalog: buildGaCatalogFromCsv(csv),
        exposeConfigured: node.etsExposeConfigured,
        exposedGAs: node.etsExposedGAs,
        readOnlyGAs: node.etsReadOnlyGAs
      })
      const snapshot = enrichCerebrumHomeCatalog(authorizedCatalog)
    ...
    const getLegacyAreaStorageFile = () => {
      ...
      return path.join(baseDir, 'cerebrum', 'areas', `cerebrum-areas-${node.id}.json`)
    }

    const getAiConfigStorageFile = () => {
      const baseDir = node.cerebrumStorageDir
      return path.join(baseDir, 'cerebrum', 'config', `cerebrum-config-${node.id}.json`)
    }

    const getHistoryArchiveDir = () => {
      const baseDir = node.cerebrumStorageDir
      return path.join(baseDir, 'cerebrum', 'history', node.id)
    }
    ...
    const getAdapterHistoryArchiveFile = dayKey => path.join(getAdapterHistoryArchiveDir(), `${String(dayKey || '').trim() || formatArchiveDayKey(Date.now())}.${CEREBRUM_COMPACT_ARCHIVE_EXTENSION}`)
    ...
    const getScheduleStorageFile = () => {
      const baseDir = node.cerebrumStorageDir
      return path.join(baseDir, 'cerebrum', 'schedules', `cerebrum-schedules-${getSafeStorageNodeId()}.json`)
    }
    ...
    const persistLastChatPromptDebug = ({ systemPrompt, staticContext, userContent } = {}) => {
      ...
      const content = [
        'Cerebrum LAST CHAT PROMPT — LOCAL DEBUG COPY',
    ...
    const persistScheduleStoreNow = () => {
      try {
        ...
        try {
          writeAtomicUtf8File({ filePath: markdownPath, content: buildCerebrumScheduleMarkdown(node._scheduleStore) })
        } catch (markdownError) {
          try { node.sysLogger?.warn(`Cerebrum schedule Markdown write error: ${markdownError.message || markdownError}`) } catch (logError) { /* ignore */ }
        }
        ...
        return {
          ...
          path: filePath,
          markdownPath,
          activeCount: listActiveCerebrumSchedules(node._scheduleStore).length
        }
      } catch (error) {
        try { node.sysLogger?.warn(`Cerebrum schedule write error: ${error.message || error}`) } catch (logError) { /* ignore */ }
        return null
      }
    }
    ...
    const cleanupHomeMemoryTempFiles = () => {
      try {
        ...
        try { node.sysLogger?.warn(`Cerebrum home memory temporary-file cleanup error: ${error.message || error}`) } catch (logError) { /* ignore */ }
      }
    }
    ...
    const loadHomeMemoryFromDisk = () => {
      ...
      try {
        ...
        let loadedMemory
        if (!fs.existsSync(filePath)) {
          loadedMemory = createEmptyCerebrumHomeMemory()
        } else {
          const stat = fs.statSync(filePath)
    ...
    const buildCerebrumMemoryFileSnapshot = ({ fromDisk = false } = {}) => {
      const filePath = getHomeMemoryFile()
      const liveMemory = normalizeCerebrumHomeMemory(node._homeMemory)
      ...
      if (fromDisk && fs.existsSync(filePath)) {
        ...
        content = buildCerebrumHomeMemoryMarkdown({ memory: liveMemory, maxKb: HOME_MEMORY_DEFAULT_KB }).markdown
      ...
      return {
        ...
        maxBytes,
        revision: buildCerebrumHomeMemoryRevision(liveMemory),
    ...
    const saveCerebrumMemoryFile = ({ content, jsonContent, revision } = {}) => {
      ...
      const currentRevision = buildCerebrumHomeMemoryRevision(node._homeMemory)
      ...
      try {
        nextMemory = hasJsonContent
          ? parseCerebrumHomeMemoryMarkdownStrict(`<!-- CEREBRUM_HOME_MEMORY_V1\n${fileContent.trim()}\nCEREBRUM_HOME_MEMORY_END -->`)
          : parseCerebrumHomeMemoryMarkdownStrict(fileContent)
      } catch (error) {
    ...
    const cleanupChatContextTempFiles = () => {
      try {
        ...
        try { node.sysLogger?.warn(`Cerebrum chat context temporary-file cleanup error: ${error.message || error}`) } catch (logError) { /* ignore */ }
      }
    }

    const persistChatContextNow = () => {
      try {
        const rendered = buildCerebrumChatContextFile({
        ...
        try { node.sysLogger?.warn(`Cerebrum chat context write error: ${error.message || error}`) } catch (logError) { /* ignore */ }
        return null
      }
    }
    ...
    const loadChatContextFromDisk = () => {
      ...
      try {
        ...
        const sharedStore = sharedCerebrumChatContextStores.get(filePath)
        if (sharedStore) {
          bindSharedCerebrumState({
        ...
        let loadedContext
        if (!fs.existsSync(filePath)) {
          loadedContext = createEmptyCerebrumChatContext()
        } else {
          const stat = fs.statSync(filePath)
    ...
    const buildChatLearningFileSnapshot = ({ fromDisk = false } = {}) => {
      const filePath = getChatContextFile()
      const liveContext = normalizeCerebrumChatContext(node._chatContext)
      let content = ''
      let stat = null
      if (fromDisk && fs.existsSync(filePath)) {
        stat = fs.statSync(filePath)
        ...
        content = buildCerebrumChatContextFile({
          context: liveContext,
          maxBytes: CHAT_CONTEXT_MAX_BYTES
        }).content
        try { if (fs.existsSync(filePath)) stat = fs.statSync(filePath) } catch (error) { /* ignore */ }
    ...
    const saveChatLearningFile = ({ content, revision } = {}) => {
      ...
      const expectedRevision = String(revision || '').trim()
      const currentRevision = buildCerebrumChatLearningRevision(node._chatContext)
      if (expectedRevision && expectedRevision !== currentRevision) {
      ...
      let nextContext
      try {
        nextContext = parseCerebrumChatContextFileStrict(fileContent)
      } catch (error) {
        throw Object.assign(new Error(error.message || String(error)), { status: 400 })
      }
      const rendered = buildCerebrumChatContextFile({
      ...
      if (!persisted) throw new Error('Unable to save the Cerebrum chat-learning file')
    ...
    const resetChatLearningFile = ({ revision } = {}) => {
      ...
      const persisted = scheduleChatContextPersist({ immediate: true })
      if (!persisted) throw new Error('Unable to reinitialize the Cerebrum chat-learning file')
      return buildChatLearningFileSnapshot({ fromDisk: true })
    }
    ...
    const pruneHistoryArchiveFiles = ({ force = false } = {}) => {
      ...
      try {
        ...
        node.sysLogger?.warn(`Cerebrum history prune error: ${error.message || error}`)
      }
    }
    ...
    const loadRecentHistoryFromDisk = () => {
      ...
      try {
        ...
        node.sysLogger?.warn(`Cerebrum history restore error: ${error.message || error}`)
      }
    }
    ...
    const loadHistoryQueryFromDisk = ({ fromTs, toTs, limit = 240, question = '', filter = null } = {}) => {
      ...
      try {
        ...
        const accumulator = createCerebrumHistoryAccumulator({ kind: 'knx', question, limit })
    ...
    const selectTelegramsForPrompt = ({ question, maxEvents }) => {
      ...
      if (node.historyStoreToDisk === true) {
        ...
        const accumulator = createCerebrumHistoryAccumulator({ kind: 'knx', question: '', limit: maxItems })
    ...
    const pruneAdapterHistoryArchiveFiles = ({ force = false } = {}) => {
      ...
      try {
        ...
        node.sysLogger?.warn(`Cerebrum adapter history prune error: ${error.message || error}`)
      }
    }
    ...
    const loadAdapterHistoryQueryFromDisk = ({ fromTs, toTs, limit = 160, question = '' } = {}) => {
      const accumulator = createCerebrumHistoryAccumulator({ kind: 'adapter', question, limit })
      const from = Number(fromTs || 0)
      const to = Number(toTs || 0)
      ...
      try {
        ...
        node.sysLogger?.warn(`Cerebrum adapter history load error: ${error.message || error}`)
      }
      return accumulator.finish()
    }
    ...
    const writePersistedAiConfig = (partialConfig) => {
      ...
      const nextConfig = {
        ...
        gaRoleExperience: partialConfig && partialConfig.gaRoleExperience && typeof partialConfig.gaRoleExperience === 'object'
          ? normalizeCerebrumGaRoleExperience(partialConfig.gaRoleExperience)
          : normalizeCerebrumGaRoleExperience(current.gaRoleExperience),
    ...
    const loadGaRoleExperience = () => {
      const current = loadPersistedAiConfig()
      return normalizeCerebrumGaRoleExperience(current && current.gaRoleExperience)
    }
      ...
      return {
        ...
        files: {
          ...
          schedulesReadable: buildCerebrumBackupFile({
            ...
            mediaType: 'text/markdown',
            fallbackContent: buildCerebrumScheduleMarkdown(node._scheduleStore)
          })
        }
      }
    }
    ...
    node.regenerateLlmAreas = async () => {
      if (!node.llmEnabled) throw new Error('LLM is disabled in the Cerebrum node config')
      ...
      try {
        ...
        node.warn(`Cerebrum role classification skipped: ${error.message || String(error)}`)
      }
    ...
    node.suggestAreaDraftWithLlm = async ({ prompt, name, description, gaList } = {}) => {
      ...
      try {
        ...
        node.warn(`Cerebrum draft GA role classification skipped: ${error.message || String(error)}`)
      }
      return {
        ok: true,
        suggestion: {
    ...
    node.getChatLearningFile = async () => {
      const persisted = scheduleChatContextPersist({ immediate: true })
      if (!persisted) throw new Error('Unable to prepare the Cerebrum chat-learning file')
      return buildChatLearningFileSnapshot({ fromDisk: true })
    }
    ...
    node.importAiConfig = async (payload) => {
      ...
      const nextGaRoles = configuration.gaRoles && typeof configuration.gaRoles === 'object'
        ...
        : {}
      const nextGaRoleExperience = Object.fromEntries(Object.entries(normalizeCerebrumGaRoleExperience(configuration.gaRoleExperience)).filter(([ga, experience]) => {
      ...
      return {
        ...
        cerebrum: {
          ...
          scheduleCount: listActiveCerebrumSchedules(node._scheduleStore).length
        }
      }
    }
    ...
    const recordChatPromptUsage = ({ body, provider, model } = {}) => {
      ...
      node._lastChatPromptUsage = Object.assign(
        ...
        measureCerebrumPromptContext({ body, provider, model })
      )
      return sequence
    }
    ...
    const callLLMChat = async ({ systemPrompt, staticContext = '', userContent, images = [], jsonSchema = null, maxTokensOverride = null, trackChatContextUsage = false, promptCacheKey = '' }) => {
      ...
      const effectiveTimeoutMs = resolveCerebrumLlmTimeoutMs({
        configuredTimeoutMs
      })
      const normalizedImages = (Array.isArray(images) ? images : []).slice(0, 1).map(image => normalizeCerebrumCameraImage(image))
      ...
      const structuredSchema = jsonSchema && jsonSchema.schema
        ? sanitizeCerebrumStructuredOutputSchema(jsonSchema.schema)
        : null
      if (node.llmProvider === 'ollama') {
        const url = resolveOllamaChatUrl(node.llmBaseUrl)
      ...
      if (useOpenAiResponses) {
        ...
        const responseBody = {
          ...
          store: false,
          truncation: 'disabled',
          prompt_cache_key: String(promptCacheKey || `cerebrum-${node.id || 'node'}`).slice(0, 64)
        }
        ...
        if (json && json._cerebrumPromptCacheOptionsUnsupported === true) {
    ...
    node.generateAiFlow = async ({ prompt, language } = {}) => {
      ...
      const userContent = [
        ...
        renderCerebrumCatalogForPrompt(catalog),
        '',
        ...
        gaLines.length ? gaLines.join('\n') : '(no ETS group address selected for Cerebrum)',
        '',
      ...
      const parsed = parseCerebrumFlowFromLlm(ret && ret.content)
      ...
      const normalized = normalizeCerebrumGeneratedFlow({
    ...
    const applyCerebrumGaRoleActions = ({ actions, sessionId } = {}) => {
    ...
    const callConversationalLLM = async ({
      ...
      let userContent = [
        ...
        routinePlanningPass ? buildCerebrumRoutineInspectionContext(routineInspection) : '',
        '',
        ...
        jsonSchema: {
          ...
          schema: {
            ...
            properties: {
              ...
              catalogActions: {
                type: 'array',
                maxItems: CEREBRUM_CATALOG_MAX_ACTIONS_PER_ROUND,
                items: {
                  type: 'object',
                  additionalProperties: false,
                  properties: {
              ...
              webActions: {
                type: 'array',
                maxItems: CEREBRUM_WEB_MAX_ACTIONS_PER_ROUND,
                items: {
                  type: 'object',
                  additionalProperties: false,
                  properties: {
              ...
              scheduleActions: {
                type: 'array',
                maxItems: CEREBRUM_SCHEDULE_MAX_ACTIONS,
                items: {
                  type: 'object',
                  additionalProperties: false,
                  properties: {
                    ...
                    instruction: { type: 'string', maxLength: CEREBRUM_SCHEDULE_MAX_INSTRUCTION_CHARS },
      ...
      const routine = safeReadOnly || webResearchStep ? normalizeCerebrumRoutineDescriptor(null) : envelope.routine
      ...
      const normalized = allowKnxCommands
        ? normalizeCerebrumCommandCandidates({
          ...
          maxReadCommands: 20,
          coercePayload: (value, context) => coerceCerebrumCommandPayload(value, context)
        })
        : { accepted: [], rejected: [] }
      ...
      ;(safeReadOnly || inspectOnly || webResearchStep ? [] : (Array.isArray(envelope.speechActions) ? envelope.speechActions : [])).slice(0, 1).forEach(action => {
        const normalizedAction = normalizeCerebrumSpeechActionCandidate(action)
    ...
    const getCerebrumWebBudgetSnapshot = () => {
      ...
      const limit = normalizeCerebrumWebMaxCallsPerHour(node.webMaxCallsPerHour)
    ...
    const executeBoundedCerebrumWebActions = async (actions, { maxActions = CEREBRUM_WEB_MAX_ACTIONS_PER_ROUND } = {}) => {
      const normalized = normalizeCerebrumWebActions(actions, { maxActions })
      if (!normalized.length) return { results: [], budget: getCerebrumWebBudgetSnapshot() }
      const budgetBefore = getCerebrumWebBudgetSnapshot()
      ...
      if (allowed.length) {
        try {
          results = await executeCerebrumWebActions(allowed, {
          ...
          try { node.sysLogger?.warn(`Cerebrum Web operation error: ${error.message || error}`) } catch (logError) { /* ignore */ }
        }
      }
    ...
    const completeCerebrumWebResearch = async ({
      initialResponse,
      question,
      sessionId,
      requireConfirmation,
      ...
      let actionCount = 0
      let rounds = 0
      let budget = getCerebrumWebBudgetSnapshot()
      while (
        response &&
        Array.isArray(response.webActions) &&
        response.webActions.length > 0 &&
        rounds < CEREBRUM_WEB_MAX_RESEARCH_ROUNDS &&
        actionCount < CEREBRUM_WEB_MAX_ACTIONS_PER_ROUND
      ) {
        const remaining = CEREBRUM_WEB_MAX_ACTIONS_PER_ROUND - actionCount
        const candidates = normalizeCerebrumWebActions(response.webActions, { maxActions: remaining })
          .filter(action => {
      ...
      return {
        ...
        results,
        sources,
        fingerprint: buildCerebrumWebResearchFingerprint(results),
        actionCount,
        rounds,
        budget,
        catalogResearchResults: accumulatedCatalogResults,
    ...
    const cloneInputMessage = inputMessage => cloneCerebrumInputMessage(
      inputMessage,
      RED.util && typeof RED.util.cloneMessage === 'function'
        ? message => RED.util.cloneMessage(message)
        : null,
      error => {
        try { node.sysLogger?.warn(`cerebrumUltimate input message clone warning: ${error.message || error}`) } catch (e) { /* ignore */ }
      }
    )
    ...
    const adaptAssistantOutput = (value, inputMessage) => {
      ...
      const adaptOne = (message) => {
        const adapted = node._chatOutputAdapter
          ...
          : message
        const redBotReadyMessage = applyCerebrumRedBotOutputEnvelopeFallback({
          preset: node.chatAdapterPreset,
          message: adapted,
          inputMessage
        })
        const outputMessage = applyCerebrumChatConfirmationPresetFallback({
          preset: node.chatAdapterPreset,
          message: applyCerebrumChatMediaPresetFallback({
            preset: node.chatAdapterPreset,
            message: applyCerebrumTelegramVoiceOutputPresetFallback({
              preset: node.chatAdapterPreset,
      ...
      try {
        ...
        try { node.sysLogger?.error(`cerebrumUltimate chat output adapter error: ${error.message || error}`) } catch (e) { /* ignore */ }
    ...
    const sendCerebrumOutputs = (outputs, inputMessage) => {
      ...
      if (sidebarCapture && Array.isArray(preparedOutputs) && preparedOutputs.length > 2 && preparedOutputs[2]) {
        ...
        const capturedMetadata = capturedMessage && capturedMessage.cerebrum && typeof capturedMessage.cerebrum === 'object'
          ? capturedMessage.cerebrum
          : {}
        const captured = {
      ...
      return safeCerebrumSend({
        ...
        onError: error => {
          try { node.sysLogger?.error(`cerebrumUltimate output error: ${error.message || error}`) } catch (e) { /* ignore */ }
          try { node.error(error, inputMessage) } catch (e) { /* ignore */ }
    ...
    const buildCerebrumReplyMessage = ({ inputMessage, content, metadata = {}, summary }) => {
      ...
      replyMessage.cerebrum = metadata
    ...
    const resolveTelegramVoiceService = () => {
      const service = resolveCerebrumVoiceServiceConfig({
      ...
      if (!service.chatCompatible) {
        ...
        error.code = 'CEREBRUM_VOICE_PROVIDER_REQUIRED'
        throw error
      }
      ...
      if ((isOfficialOpenAiVoiceUrl(transcriptionUrl) || isOfficialOpenAiVoiceUrl(speechUrl)) && !service.apiKey) {
        ...
        error.code = 'CEREBRUM_VOICE_API_KEY_REQUIRED'
        throw error
      }
      return Object.assign({}, service, { transcriptionUrl, speechUrl })
    }

    const prepareCerebrumTelegramVoiceInput = async (message) => {
      if (!isCerebrumTelegramVoiceInput(message)) return message
      ...
      redactCerebrumTelegramVoiceLocations(message)
      if (!node.llmEnabled) {
        const error = new Error('Telegram voice messages require the LLM to be enabled')
        error.code = 'CEREBRUM_VOICE_LLM_DISABLED'
        throw error
      }
      const voiceService = resolveTelegramVoiceService()
      const audio = await fetchCerebrumTelegramVoice({
        voiceInput,
        timeoutMs: CEREBRUM_VOICE_API_TIMEOUT_MS
      })
      const transcription = await postCerebrumVoiceTranscription({
      ...
      message.cerebrum = Object.assign({}, message.cerebrum, { voiceInput: safeVoiceInput })
      return message
    }

    const enrichCerebrumTelegramVoiceReplyMetadata = async ({ inputMessage, content, speechContent, metadata = {} } = {}) => {
      const enriched = Object.assign({}, metadata)
      if (!isCerebrumTelegramVoiceInput(inputMessage) || !['windkh-telegrambot', 'redbot-telegram'].includes(node.chatAdapterPreset)) return enriched
      ...
      try {
        ...
        const voiceService = resolveTelegramVoiceService()
        const audio = await postCerebrumVoiceSpeech({
        ...
        try { node.sysLogger?.warn(`Cerebrum Telegram voice reply fallback: ${error.message || error}`) } catch (logError) { /* ignore */ }
      }
      return enriched
    }

    const buildCerebrumVoiceAwareReplyMessage = async ({ inputMessage, content, speechContent, metadata = {}, summary }) => {
      return buildCerebrumReplyMessage({
        inputMessage,
        content,
        metadata: await enrichCerebrumTelegramVoiceReplyMetadata({ inputMessage, content, speechContent, metadata }),
        summary
      })
    }

    const startCerebrumThinkingFeedback = ({ inputMessage, question, sessionId, language }) => {
      let timer = null
      const stop = () => {
        if (!timer) return
        clearTimeout(timer)
      ...
      timer = setTimeout(() => {
        ...
        const replyMessage = buildCerebrumReplyMessage({
          inputMessage,
          content: getCerebrumThinkingCopy(language),
          metadata: {
            type: 'thinking',
            transient: true,
            question,
        ...
        sendCerebrumOutputs([null, null, replyMessage, null], inputMessage)
      }, CEREBRUM_THINKING_DELAY_MS)
      node._thinkingTimers.add(timer)
      return stop
    }

    const buildCerebrumCommandMessages = ({ commands, question, sessionId, confirmed, inputMessage }) => {
      return (Array.isArray(commands) ? commands : []).map((command, index) => buildCerebrumUniversalMessage({
        command,
        question,
        sessionId,
        confirmed,
    ...
    const executeCerebrumReadOperations = async ({ commands, question, sessionId, inputMessage, language }) => {
      const operations = (Array.isArray(commands) ? commands : [])
      ...
      const messages = buildCerebrumCommandMessages({
        commands: operations,
        question,
        sessionId,
        confirmed: false,
      ...
      if (!sendCerebrumOutputs([null, null, null, messages], inputMessage)) {
      ...
      const metadata = buildCerebrumReadResultMetadata({ operations, results })
      return {
        sent: true,
        operations,
        results,
        metadata,
        text: formatCerebrumReadResults({ operations, results, language })
      }
    }
    ...
    const buildCameraSyntheticInput = ({ sessionId, language }) => {
      ...
      return {
        ...
        cerebrum: { sessionId: String(sessionId || 'default') }
      }
    }
    ...
    const emitCameraChatReply = ({ inputMessage, content, metadata, image }) => {
      ...
      const reply = buildCerebrumReplyMessage({
        inputMessage,
        content,
        metadata: cameraMetadata
      })
      return sendCerebrumOutputs([null, null, reply, null], inputMessage)
    }
    ...
    const isPendingScheduledCameraAuthorized = (pending) => {
      ...
      const task = normalizeCerebrumScheduleStore(node._scheduleStore).tasks.find(item => item.id === taskId)
    ...
    const finishPendingCameraRequest = async (msg) => {
      const meta = msg && msg.cerebrum ? msg.cerebrum : {}
      ...
      let image
      try {
        image = normalizeCerebrumCameraImage({
          data: msg.payload,
    ...
    const startCameraSnapshotRequest = ({ action, sessionId, inputMessage, question, language, caption, notificationEvent, webSources = [], webMetadata = null, scheduledTask = null, scheduledNotificationFingerprint = '' }) => {
      ...
      pending.timer = setTimeout(() => {
        ...
        const fallback = notificationEvent
          ? buildCerebrumCameraNotificationText({ language: pending.language, event: notificationEvent })
    ...
    const describeCameraWatches = ({ sessionId, language }) => {
      ...
      const watches = listCerebrumCameraWatches(node._chatContext, sessionId)
    ...
    const buildTtsUltimateSpeechOutput = ({ actions, sessionId }) => {
      ...
      ;(Array.isArray(actions) ? actions : []).slice(0, 1).forEach(action => {
        try {
          const message = buildCerebrumTtsUltimateAnnouncementMessage({
          ...
          sent.push({
            ...
            reason: message.cerebrum.reason
          })
        } catch (error) {
    ...
    const formatCerebrumScheduleResults = ({ results, language }) => {
    ...
    const deferClaimedScheduledTask = ({ taskId, delayMs = 60 * 1000, reason = 'another chat operation is still active' } = {}) => {
      const now = nowMs()
      const previousStore = normalizeCerebrumScheduleStore(node._scheduleStore, { now })
      const store = normalizeCerebrumScheduleStore(previousStore, { now })
      ...
      node._scheduleStore = normalizeCerebrumScheduleStore(store, { now })
    ...
    const applyScheduleActions = ({ actions, sessionId, language, sourceRequest }) => {
      ...
      if (changed && !scheduleScheduleStorePersist({ immediate: true })) {
        node._scheduleStore = previousStore
        const activeBefore = listActiveCerebrumSchedules(previousStore, { sessionId })
      ...
      return {
        results,
        additions: formatCerebrumScheduleResults({ results, language })
      }
    }
    ...
    const applyCameraActions = ({ actions, sessionId, inputMessage, question, language, reply, webSources = [], webMetadata = null, scheduledTask = null, scheduledNotificationFingerprint = '' }) => {
      ...
      list.forEach((action) => {
        ...
        if (action.type === 'unwatch') {
          ...
          const removal = removeCerebrumCameraWatches(node._chatContext, {
            sessionId,
            predicate: watch => {
        ...
        if (typeof provider.subscribe === 'function') {
          const unsubscribe = provider.subscribe(event => {
            try { handleHomeAutomationAdapterEvent(event, provider) } catch (error) {
              try { node.sysLogger?.warn(`Cerebrum home automation event error: ${error.message || error}`) } catch (logError) { /* ignore */ }
            }
          })
    ...
    const determineHomeAssistantRefreshSeconds = () => {
      const states = normalizeCerebrumHomeMemory(node._homeMemory).states.filter(item => item.source === 'home-assistant')
    ...
    const refreshCerebrumKnxStates = now => {
      ...
      const dueStates = normalizeCerebrumHomeMemory(node._homeMemory).states
    ...
    const emitCerebrumBootNotification = async () => {
      ...
      const syntheticInputMessage = {
        ...
        language,
        boot: true,
        cerebrum: { type: 'boot_notification', boot: true, sessionId }
      }
      const metadata = {
    ...
    const handleCerebrumConfirmationDecision = async ({ msg, question, sessionId, decision }) => {
      const pending = node._pendingKnxCommands.get(sessionId)
      const language = pending && pending.language
        ? pending.language
        : resolveCerebrumLanguage(msg, 'en', question)
      const copy = getCerebrumConfirmationCopy(language)
      if (!pending) {
        const reply = await buildCerebrumVoiceAwareReplyMessage({
          inputMessage: msg,
          content: copy.missing,
          metadata: { type: 'knx_confirmation_missing', sessionId }
        })
        sendCerebrumOutputs([null, null, reply, null], msg)
        return
      }
      ...
      if (decision !== 'cancel' && pending.scheduledTaskId) {
        const scheduledTask = normalizeCerebrumScheduleStore(node._scheduleStore).tasks
      ...
      const routine = normalizeCerebrumRoutineDescriptor(pending.routine)
      const normalized = normalizeCerebrumCommandCandidates({
      ...
      const commandMessages = buildCerebrumCommandMessages({
        commands: normalized.accepted,
        question: pending.question,
        sessionId,
        confirmed: true,
      ...
      if (routine.active) {
        ...
        const feedbackWaiters = normalized.accepted.map(command => waitForTelegram({
          ...
          timeoutMs: CEREBRUM_ROUTINE_FEEDBACK_TIMEOUT_MS,
        ...
        const startedReply = buildCerebrumReplyMessage({
          inputMessage: msg,
        ...
        const finalContent = speechActionResult.errors.length
          ...
          : report.text
        const finalReply = await buildCerebrumVoiceAwareReplyMessage({
          inputMessage: msg,
        ...
        if (!sendCerebrumOutputs([null, null, finalReply, null, speechActionResult.messages.length ? speechActionResult.messages : null], msg)) return
      ...
      rememberConversationTurn({ sessionId, question: question || 'CONFIRM', reply: content })
      if (isCerebrumTelegramVoiceInput(msg)) {
        if (!sendCerebrumOutputs([null, null, null, commandMessages], msg)) return
        const reply = await buildCerebrumVoiceAwareReplyMessage({ inputMessage: msg, content, metadata: replyMetadata })
        if (!sendCerebrumOutputs([null, null, reply, null], msg)) return
      } else {
    ...
    const emitSummary = () => {
      try {
        const summary = rebuildCachedSummaryNow()
        node.send([{ topic: node.outputtopic, payload: summary, cerebrum: { type: 'summary' } }, null, null])
        ...
        try { node.sysLogger?.error(`cerebrumUltimate emitSummary error: ${error.message || error}`) } catch (e) { /* ignore */ }
    ...
    const rememberHomeOwner = ({ sessionId, language } = {}) => {
      try {
        ...
        try { node.sysLogger?.warn(`Cerebrum home owner memory error: ${error.message || error}`) } catch (logError) { /* ignore */ }
      }
    }
    ...
    const emitCerebrumOnboarding = (msg) => {
      const question = extractCerebrumQuestion(msg)
      const sessionId = resolveCerebrumSessionId(msg)
      const language = resolveCerebrumLanguage(msg, 'en', question)
      ...
      if (!sendCerebrumOutputs([null, null, replyMessage, null, null], msg)) return false
    ...
    const handleCommand = async (msg) => {
      try {
        ...
        if (cmd === 'reset') {
          ...
          node.send([{ topic: node.outputtopic, payload: { ok: true, schedulesReset }, cerebrum: { type: 'reset', schedulesReset } }, null, null])
          return
        }
        ...
        if (cmd === 'ask') {
          const question = extractCerebrumQuestion(msg)
          const sessionId = resolveCerebrumSessionId(msg)
          const scheduledTask = msg && msg.cerebrum && msg.cerebrum.scheduledTask && typeof msg.cerebrum.scheduledTask === 'object'
            ? msg.cerebrum.scheduledTask
            : null
          const scheduledTaskRun = !!(scheduledTask && scheduledTask.id)
          const backgroundExecution = scheduledTaskRun
          const sidebarRequest = !!(msg && msg.cerebrum && msg.cerebrum.sidebarRequestId)
          if (!question) throw new Error('Missing question')
          if (!backgroundExecution && isCerebrumOnboardingRequest({ msg, question, topic: cmd })) {
            emitCerebrumOnboarding(msg)
            return
          }
          const decision = classifyCerebrumConfirmation({ msg, question, topic: cmd })
          ...
          try {
            ...
            const stopThinkingFeedback = backgroundExecution || sidebarRequest
              ? () => {}
              : startCerebrumThinkingFeedback({
                inputMessage: msg,
                question,
                sessionId,
                language: requestLanguage
              })
            const safeReadOnly = isCerebrumSafeFirstRunPrompt(question)
            let ret
            ...
            let webResearch = {
              ...
              actionCount: 0,
              rounds: 0,
              budget: getCerebrumWebBudgetSnapshot()
            }
            try {
              ...
              if (Array.isArray(ret && ret.webActions) && ret.webActions.length > 0) {
                webResearch = await completeCerebrumWebResearch({
                  initialResponse: ret,
                  question,
                  sessionId,
                  requireConfirmation: node.llmRequireCommandConfirmation,
              ...
              const initialRoutine = normalizeCerebrumRoutineDescriptor(ret && ret.routine)
              const inspectionCommands = (Array.isArray(ret && ret.commands) ? ret.commands : [])
                .filter(command => command && command.event === 'GroupValue_Read')
              if (initialRoutine.active && inspectionCommands.length > 0) {
                const inspectionLanguage = resolveCerebrumLanguage(msg, requestLanguage, question, ret.language)
                const inspection = await executeCerebrumReadOperations({
                ...
                const plannedRoutine = normalizeCerebrumRoutineDescriptor(planned && planned.routine)
            ...
            const language = resolveCerebrumLanguage(msg, requestLanguage, question, ret.language)
            ...
            if (scheduledTaskRun) {
              const liveTask = normalizeCerebrumScheduleStore(node._scheduleStore).tasks.find(task => task.id === scheduledTask.id)
              ...
              if (cancelled || duplicateFingerprint) {
                if (!cancelled) {
                  const completion = completeCerebrumScheduleRun({ store: node._scheduleStore, taskId: scheduledTask.id, ok: true })
                  node._scheduleStore = completion.store
                  scheduleScheduleStorePersist({ immediate: true })
                }
            ...
            const appliedMemoryActions = applyCerebrumMemoryActions({
            ...
            const rejectedScheduleAdditions = formatCerebrumScheduleResults({
            ...
            if (speechActionResult.messages.length && !sendCerebrumOutputs([null, null, null, null, speechActionResult.messages], msg)) return
            ...
            if (awaitingConfirmation) {
              ...
              confirmationRequest = buildCerebrumConfirmationRequest({
                sessionId,
                expiresAt,
            ...
            if (emittedReadCommands.length > 0) {
              ...
              if (scheduledTaskRun) {
                const liveAfterRead = normalizeCerebrumScheduleStore(node._scheduleStore).tasks.find(task => task.id === scheduledTask.id)
              ...
              const readReply = formatCerebrumReadResults({
            ...
            if (!commandMessagesSent && commandMessages.length > 0 && isCerebrumTelegramVoiceInput(msg)) {
              if (!sendCerebrumOutputs([null, null, null, commandMessages], msg)) return
              commandMessagesSent = true
            }
            ...
            const replyMessage = deferCameraReply
              ? null
              : await buildCerebrumVoiceAwareReplyMessage({
                inputMessage: msg,
                content,
            ...
            if (scheduledTaskRun) {
              const liveBeforeReply = normalizeCerebrumScheduleStore(node._scheduleStore).tasks.find(task => task.id === scheduledTask.id)
            ...
            if (deferCameraReply) {
              ...
              if (!sendCerebrumOutputs([null, null, replyMessage, null], msg)) return
            ...
            updateConversationStatus({
              type: 'request',
              question,
              language: resolveCerebrumLanguage(msg, 'en', question)
            })
            if (node._closing === true) return
            if (interactiveRequestToken && node._interactiveChatRequests.get(sessionId) !== interactiveRequestToken) return
            if (!sendCerebrumOutputs([null, null, replyMessage, null], msg)) return
        ...
        if (cmd === 'clear_chat') {
          ...
          node._chatContext = clearCerebrumChatSession(node._chatContext, sessionId)
          node._pendingKnxCommands.delete(sessionId)
          const scheduleStoreBeforeReset = normalizeCerebrumScheduleStore(node._scheduleStore)
          const scheduleReset = applyCerebrumScheduleActions({
        ...
        if (cmd === 'run_profile') {
          ...
          node.send([{
            topic: node.outputtopic,
            payload: ret.report,
            cerebrum: { type: 'profile_report', areaId, profileId }
          }, null, null])
        ...
        if (cmd === 'run_actuator_test') {
          ...
          node.send([{
            topic: node.outputtopic,
            payload: ret.report,
            cerebrum: { type: 'actuator_test_report', commandGA: ret.report.command.ga }
          }, null, null])
        ...
        if (cmd === 'run_test_plan') {
          ...
          node.send([{
            topic: node.outputtopic,
            payload: ret.report,
            cerebrum: { type: 'ai_test_plan_report', areaId: ret.report && ret.report.area ? ret.report.area.id : '' }
          }, null, null])
          updateStatus({ fill: 'green', shape: 'dot', text: `Test plan ${ret.report.name || ret.report.id}` })
          return
        }

        node.warn(`cerebrumUltimate: unknown command '${cmd}'. Supported: reset, summary, ask, confirm, cancel, clear_chat, run_profile, run_actuator_test, run_test_plan`)
      } catch (error) {
        try { node.sysLogger?.error(`cerebrumUltimate handleCommand error: ${error.message || error}`) } catch (e) { /* ignore */ }
        try { node.error(error) } catch (e) { /* ignore */ }
    ...
    const buildScheduledTaskSyntheticInput = (task) => {
      ...
      synthetic.cerebrum = Object.assign({}, synthetic.cerebrum, {
        type: 'scheduled_task_execution',
        sessionId,
        scheduledTask: task
      })
      delete synthetic.cerebrum.voiceInput
      delete synthetic.cerebrum.sidebarRequestId
    ...
    const runScheduledTaskTick = async () => {
      ...
      if (getLivePendingKnxCommands(nextDue.sessionId, now) || node._interactiveChatRequests.has(nextDue.sessionId)) {
        const storeBeforeDefer = normalizeCerebrumScheduleStore(node._scheduleStore, { now })
      ...
      const storeBeforeClaim = normalizeCerebrumScheduleStore(node._scheduleStore, { now })
      const claim = claimDueCerebrumSchedules({ store: storeBeforeClaim, now, limit: 1 })
    ...
    node.getSidebarState = ({ fresh = false, language = 'en' } = {}) => {
      try {
        ...
        return {
          ...
          schedules: listActiveCerebrumSchedules(node._scheduleStore)
        }
      } catch (error) {
        return {
          node: {
    ...
    node.sidebarAsk = async (question) => {
      ...
      const language = resolveCerebrumLanguage({}, 'en', q)
        ...
        try {
          return await Promise.race([
            ...
            new Promise((resolve, reject) => {
              timeout = setTimeout(() => reject(new Error('The Cerebrum sidebar request did not produce a reply')), 25000)
            })
          ])
        } finally {
    ...
    const processCerebrumInput = async (msg) => {
      ...
      if (!adaptedMessage) {
        adaptedMessage = applyCerebrumTelegramVoiceInputPresetFallback({
          preset: node.chatAdapterPreset,
          message: msg
        })
      }
      ...
      if (isCerebrumTelegramVoiceInput(adaptedMessage)) {
        try {
          ...
          const language = resolveCerebrumLanguage(adaptedMessage, 'en', '')
          const genericCopies = {
          ...
          const providerCopies = {
            en: 'Telegram voice requires the OpenAI-compatible chat provider. Select it in Cerebrum → AI Assistant Connection and deploy.',
            it: 'I vocali Telegram richiedono il provider chat OpenAI-compatible. Selezionalo in Cerebrum → Connessione Assistente AI e fai Deploy.',
            de: 'Telegram-Sprachnachrichten erfordern den OpenAI-kompatiblen Chat-Provider. Wählen Sie ihn unter Cerebrum → Verbindung zum KI-Assistenten aus und führen Sie Deploy aus.',
            fr: 'Les messages vocaux Telegram nécessitent le fournisseur de chat OpenAI-compatible. Sélectionnez-le dans Cerebrum → Connexion de l’assistant IA, puis déployez.',
            es: 'Los mensajes de voz de Telegram requieren el proveedor de chat OpenAI-compatible. Selecciónalo en Cerebrum → Conexión del Asistente IA y vuelve a desplegar.',
            zh: 'Telegram 语音消息需要 OpenAI-compatible 聊天提供商。请在 Cerebrum → AI 助手连接中选择它，然后重新部署。'
          }
          const apiKeyCopies = {
            en: 'Telegram voice needs the API key of the selected OpenAI-compatible provider. Enter it in Cerebrum → AI Assistant Connection and deploy.',
            it: 'I vocali Telegram richiedono la API key del provider OpenAI-compatible selezionato. Inseriscila in Cerebrum → Connessione Assistente AI e fai Deploy.',
            de: 'Telegram-Sprachnachrichten benötigen den API-Schlüssel des ausgewählten OpenAI-kompatiblen Providers. Tragen Sie ihn unter Cerebrum → Verbindung zum KI-Assistenten ein und führen Sie Deploy aus.',
            fr: 'Les messages vocaux Telegram nécessitent la clé API du fournisseur OpenAI-compatible sélectionné. Saisissez-la dans Cerebrum → Connexion de l’assistant IA, puis déployez.',
            es: 'Los mensajes de voz de Telegram necesitan la clave API del proveedor OpenAI-compatible seleccionado. Introdúcela en Cerebrum → Conexión del Asistente IA y vuelve a desplegar.',
            zh: 'Telegram 语音消息需要所选 OpenAI-compatible 提供商的 API 密钥。请在 Cerebrum → AI 助手连接中填写，然后重新部署。'
          }
          const copySet = error && error.code === 'CEREBRUM_VOICE_PROVIDER_REQUIRED'
            ? providerCopies
            : error && error.code === 'CEREBRUM_VOICE_API_KEY_REQUIRED'
              ? apiKeyCopies
              : genericCopies
          ...
          const replyMessage = buildCerebrumReplyMessage({
            ...
            metadata: {
              ...
              sessionId: resolveCerebrumSessionId(adaptedMessage),
              language,
              errorCode: error && error.code ? String(error.code) : '',
              error: error.message || String(error)
            }
          })
          try { node.sysLogger?.warn(`Cerebrum Telegram voice input error: ${error.message || error}`) } catch (logError) { /* ignore */ }
          ...
          sendCerebrumOutputs([null, null, replyMessage, null], adaptedMessage)
          return
        }
      }
      const adaptedTopic = String(adaptedMessage.topic || '').toLocaleLowerCase()
      const requestText = extractCerebrumQuestion(adaptedMessage) || adaptedTopic || 'input'
      const requestLanguage = resolveCerebrumLanguage(adaptedMessage, 'en', requestText)
    ...
    node.on('input', function (msg) {
      processCerebrumInput(msg).catch((error) => {
        try { node.sysLogger?.error(`cerebrumUltimate input error: ${error.message || error}`) } catch (e) { /* ignore */ }
        try { node.error(error, msg) } catch (e) { /* ignore */ }
      })
    })
          ...
          node._sidebarAskCaptures.forEach(capture => {
            try {
              if (capture && typeof capture.resolve === 'function') {
                capture.resolve({ answer: 'Cerebrum node closed before the request completed.', provider: '', model: '', metadata: { type: 'node_closed' } })
              }
            } catch (error) { /* ignore */ }
          })
    ...
    try {
      ...
      Promise.resolve(syncCameraAdapterRegistry({ force: true })).catch(error => {
        try { node.sysLogger?.warn(`Cerebrum camera adapter startup error: ${error.message || error}`) } catch (logError) { /* ignore */ }
      })
    ...
    node._bootAssistantTimer = setTimeout(() => {
      node._bootAssistantTimer = null
      if (node._closing === true) return
      Promise.resolve(emitCerebrumBootNotification()).catch(error => {
        try { node.sysLogger?.warn(`Cerebrum startup notification error: ${error.message || error}`) } catch (logError) { /* ignore */ }
      })
    }, 2500)
...
module.exports.__test = {
  ...
  CEREBRUM_VOICE_SPEECH_VOICE,
  CEREBRUM_VOICE_TRANSCRIPTION_MODEL,
  CEREBRUM_WEB_MAX_ACTIONS_PER_ROUND,
  CEREBRUM_WEB_MAX_RESEARCH_ROUNDS,
  CEREBRUM_WEB_MAX_SOURCES,
  bindSharedCerebrumState,
  applyCerebrumCatalogAccessConfiguration,
  applyCerebrumChatConfirmationPresetFallback,
  applyCerebrumChatMediaPresetFallback,
  applyCerebrumRedBotOutputEnvelopeFallback,
  applyCerebrumTelegramVoiceInputPresetFallback,
  applyCerebrumTelegramVoiceOutputPresetFallback,
  applyCerebrumGaRoleActionsToCatalog,
  buildCerebrumConversationMemoryAnchor,
  buildGaCatalogFromCsv,
  buildCerebrumWebResearchContext,
  buildCerebrumWebResearchFingerprint,
  buildCerebrumFirstRunExperience,
  buildCerebrumChatLearningRevision,
  ...
  cloneCerebrumInputMessage,
  compileCerebrumChatAdapter,
  coerceCerebrumCommandPayload,
  detectCerebrumLanguageFromText,
  deriveOpenAiCompatibleAudioUrl,
  ...
  extractCerebrumQuestion,
  estimateCerebrumLogicalFunctions,
  fetchCerebrumTelegramVoice,
  formatCerebrumCommandPreview,
  formatCerebrumReadResults,
  formatCerebrumRoutineExecutionReport,
  getCerebrumConfirmationCopy,
  getCerebrumBootFallbackCopy,
  getCerebrumReadCopy,
  getCerebrumRequestStatusLabel,
  getCerebrumThinkingCopy,
