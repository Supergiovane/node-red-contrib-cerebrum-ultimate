...
function createBackupZipStream (backup) {
  ...
  entries.set('README.txt', Buffer.from([
    ...
    'Install the packages in required-packages.json. cerebrum-flows.json contains only sanitized settings for the selected Cerebrum node: it has no tab, wiring, referenced config nodes or credentials.',
    'Installa i pacchetti in required-packages.json. cerebrum-flows.json contiene solo le impostazioni sanitizzate del nodo Cerebrum selezionato: non include tab, collegamenti, config-node referenziati o credenziali.',
    'Import or create that Cerebrum node, reconnect integrations manually, deploy, then restore this ZIP. Re-enter all credentials and verify external service settings.',
    'Importa o crea quel nodo Cerebrum, ricollega manualmente le integrazioni, esegui il deploy e poi ripristina questo ZIP. Reinserisci tutte le credenziali e verifica i servizi esterni.',
    '',
    ...
    'Node-RED integration credentials are excluded. The backup still contains private conversations and home data: keep it secure.',
    'Le credenziali delle integrazioni Node-RED sono escluse. Il backup contiene comunque conversazioni e dati domestici privati: conservalo in modo sicuro.',
    '',
