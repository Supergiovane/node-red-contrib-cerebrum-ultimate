const CEREBRUM_SCHEDULE_STORE_VERSION = 1
const CEREBRUM_SCHEDULE_MAX_ACTIVE_TASKS = 32
const CEREBRUM_SCHEDULE_MAX_STORED_TASKS = 64
const CEREBRUM_SCHEDULE_MAX_ACTIONS = 8
const CEREBRUM_SCHEDULE_MIN_INTERVAL_MINUTES = 5
const CEREBRUM_SCHEDULE_MAX_INTERVAL_MINUTES = 365 * 24 * 60
const CEREBRUM_SCHEDULE_MAX_INSTRUCTION_CHARS = 4000
const CEREBRUM_SCHEDULE_ONE_TIME_LEASE_MS = 60 * 1000
...
const normalizeIntervalMinutes = (value) => {
  ...
  return Math.max(
    CEREBRUM_SCHEDULE_MIN_INTERVAL_MINUTES,
    Math.min(CEREBRUM_SCHEDULE_MAX_INTERVAL_MINUTES, Math.round(numeric))
  )
}

const createEmptyCerebrumScheduleStore = () => ({
  version: CEREBRUM_SCHEDULE_STORE_VERSION,
  createdAt: new Date().toISOString(),
  updatedAt: new Date().toISOString(),
  tasks: []
})

const normalizeCerebrumScheduleTask = (value, { now = Date.now() } = {}) => {
  const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {}
  const id = clampText(source.id, 96).replace(/[^A-Za-z0-9_.-]/g, '')
  const instruction = clampText(source.instruction, CEREBRUM_SCHEDULE_MAX_INSTRUCTION_CHARS)
  ...
  return {
    ...
    instruction,
    sourceRequest: clampText(source.sourceRequest, CEREBRUM_SCHEDULE_MAX_INSTRUCTION_CHARS),
...
const normalizeCerebrumScheduleStore = (value, { now = Date.now() } = {}) => {
  const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {}
  const seen = new Set()
  const tasks = (Array.isArray(source.tasks) ? source.tasks : [])
    .map(task => normalizeCerebrumScheduleTask(task, { now }))
    .filter(task => {
  ...
  const bounded = active.concat(inactive.slice(0, Math.max(0, CEREBRUM_SCHEDULE_MAX_STORED_TASKS - active.length)))
    .slice(0, CEREBRUM_SCHEDULE_MAX_STORED_TASKS)
  return {
    version: CEREBRUM_SCHEDULE_STORE_VERSION,
    createdAt: toIso(source.createdAt) || new Date(Number(now)).toISOString(),
...
const normalizeCerebrumScheduleActions = (value, { now = Date.now() } = {}) => {
  const accepted = []
  const rejected = []
  ;(Array.isArray(value) ? value : []).slice(0, CEREBRUM_SCHEDULE_MAX_ACTIONS).forEach((candidate, index) => {
    ...
    const instruction = clampText(source.instruction || source.task || source.text, CEREBRUM_SCHEDULE_MAX_INSTRUCTION_CHARS)
...
const applyCerebrumScheduleActions = ({ store, actions, sessionId = 'default', language = 'en', sourceRequest = '', now = Date.now(), idFactory } = {}) => {
  const target = normalizeCerebrumScheduleStore(store, { now })
  const normalized = normalizeCerebrumScheduleActions(actions, { now })
  ...
  normalized.accepted.forEach(action => {
    ...
    if (activeCount >= CEREBRUM_SCHEDULE_MAX_ACTIVE_TASKS) {
      results.push({ operation: 'create', ok: false, error: `the active schedule limit (${CEREBRUM_SCHEDULE_MAX_ACTIVE_TASKS}) has been reached` })
      return
    }
    let id
    try {
    ...
    existingIds.add(id)
    const task = normalizeCerebrumScheduleTask({
      id,
      kind: action.kind,
      title: action.title,
      instruction: action.instruction,
      sourceRequest: clampText(sourceRequest, CEREBRUM_SCHEDULE_MAX_INSTRUCTION_CHARS),
  ...
  return {
    store: normalizeCerebrumScheduleStore(target, { now }),
    results,
...
const claimDueCerebrumSchedules = ({ store, now = Date.now(), limit = 1 } = {}) => {
  const target = normalizeCerebrumScheduleStore(store, { now })
  ...
  due.forEach(task => {
    ...
    if (intervalMs > 0) {
      ...
      task.nextRunAt = task.kind === 'command'
        ? ''
        : new Date(Number(now) + CEREBRUM_SCHEDULE_ONE_TIME_LEASE_MS).toISOString()
    }
    claimed.push(clonePlain(task, {}))
  })
  target.updatedAt = new Date(Number(now)).toISOString()
  return { store: normalizeCerebrumScheduleStore(target, { now }), claimed }
}

const completeCerebrumScheduleRun = ({ store, taskId, ok = true, error = '', notified = false, notificationFingerprint = '', now = Date.now() } = {}) => {
  const target = normalizeCerebrumScheduleStore(store, { now })
  ...
  target.updatedAt = new Date(Number(now)).toISOString()
  return { store: normalizeCerebrumScheduleStore(target, { now }), task: clonePlain(task, {}) }
}

const listActiveCerebrumSchedules = (store, { sessionId, now = Date.now() } = {}) => {
  const owner = sessionId === undefined ? null : String(sessionId || 'default')
  return normalizeCerebrumScheduleStore(store, { now }).tasks
    .filter(task => task.status === 'active' && (owner === null || task.sessionId === owner))
    .sort((left, right) => String(left.nextRunAt).localeCompare(String(right.nextRunAt)))
    .map(task => clonePlain(task, {}))
}

const buildCerebrumSchedulePromptContext = (store, { sessionId, maxChars = 12000, now = Date.now() } = {}) => {
  const tasks = listActiveCerebrumSchedules(store, { sessionId, now })
...
const buildCerebrumScheduleMarkdown = (store, { now = Date.now() } = {}) => {
  const target = normalizeCerebrumScheduleStore(store, { now })
  const lines = [
    '# Cerebrum Plans and Reminders',
    '',
    'This file is generated by Cerebrum. Manage these tasks in natural-language chat; the JSON file is the authoritative runtime state.',
    '',
...
module.exports = {
  ...
  CEREBRUM_SCHEDULE_MAX_ACTIVE_TASKS,
  CEREBRUM_SCHEDULE_MAX_INSTRUCTION_CHARS,
  CEREBRUM_SCHEDULE_MAX_INTERVAL_MINUTES,
  CEREBRUM_SCHEDULE_MAX_STORED_TASKS,
  CEREBRUM_SCHEDULE_MIN_INTERVAL_MINUTES,
  CEREBRUM_SCHEDULE_ONE_TIME_LEASE_MS,
  CEREBRUM_SCHEDULE_STORE_VERSION,
