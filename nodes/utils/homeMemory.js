...
const buildCerebrumHabitId = habit => {
...
const inferCerebrumHomeSemantic = (item = {}) => {
  const source = [
    item.mainGroup,
    item.middleGroup,
    item.hierarchyPath,
...
const enrichCerebrumHomeCatalog = (catalog) => (Array.isArray(catalog) ? catalog : []).map(item => {
  return Object.assign({}, item, {
    semantic: inferCerebrumHomeSemantic(item)
  })
})

const createEmptyCerebrumHomeMemory = () => ({
...
const normalizeReconciler = value => {
  ...
  return {
    ...
    autonomousReadCount: Math.max(0, Number(source.autonomousReadCount) || 0),
    lastError: clampText(source.lastError, 500)
  }
}

const normalizeCerebrumHomeMemory = (value = {}) => {
  const source = value && typeof value === 'object' && !Array.isArray(value) ? value : {}
  return {
    version: HOME_MEMORY_VERSION,
...
const addBoundedCerebrumNotification = (memory, notification) => {
  const target = normalizeCerebrumHomeMemory(memory)
...
const addBoundedCerebrumHabitDecision = (memory, decision) => {
  const target = normalizeCerebrumHomeMemory(memory)
...
const resolveCerebrumStateTier = ({ observedAt, changedAt, observations, changes, now = Date.now() } = {}) => {
...
const finalizeCerebrumCurrentStates = target => {
...
const markCerebrumStateRefreshRequested = (memory, { key, at, retrySeconds = 300 } = {}) => {
  const target = normalizeCerebrumHomeMemory(memory)
...
const updateCerebrumReconciler = (memory, patch = {}) => {
  const target = normalizeCerebrumHomeMemory(memory)
...
const updateCerebrumCoverHabit = (memory, { ga, label, area, durationMinutes, at } = {}) => {
  const target = normalizeCerebrumHomeMemory(memory)
...
const updateCerebrumTemporalHabit = (memory, {
  ...
  const next = {
    id: existing && existing.id ? existing.id : buildCerebrumHabitId({ type: 'temporal_state_pattern', source: normalizedSource, objectId: normalizedObjectId, value: normalizedValue, dayType, timeBucket }),
...
const findCerebrumHabitCandidates = (memory, {
...
} = {}) => normalizeCerebrumHomeMemory(memory).habits
...
const applyCerebrumHabitDecision = (memory, { habitId, operation, userMessage, userOverride, at, sessionId } = {}) => {
  let target = normalizeCerebrumHomeMemory(memory)
  ...
  target = addBoundedCerebrumHabitDecision(target, {
    at: decidedAt,
    habitId: habit.id,
...
const findCerebrumHabitPredictions = (memory, { date = new Date(), windowMinutes = 30, minSamples = 5, minConfidence = 0.45 } = {}) => {
  const target = normalizeCerebrumHomeMemory(memory)
...
const buildCerebrumHomeMemoryMarkdown = ({ memory, maxKb } = {}) => {
  ...
  const render = () => {
    ...
    const lines = [
      ...
      '',
      '## AI Education — configured on the Cerebrum node',
      '',
    ...
    lines.push(`- Autonomous adapter reads: ${Number(bounded.reconciler.autonomousReadCount || 0)}`)
...
const parseCerebrumHomeMemoryMarkdown = (markdown) => {
  const text = String(markdown || '')
  const match = text.match(/<!-- CEREBRUM_HOME_MEMORY_V1\s*\n([\s\S]*?)\nCEREBRUM_HOME_MEMORY_END -->/)
  if (!match) return createEmptyCerebrumHomeMemory()
  try {
    return normalizeCerebrumHomeMemory(JSON.parse(match[1]))
  } catch (error) {
    return createEmptyCerebrumHomeMemory()
  }
}

const parseCerebrumHomeMemoryMarkdownStrict = markdown => {
  const text = String(markdown || '')
  const match = text.match(/<!-- CEREBRUM_HOME_MEMORY_V1\s*\n([\s\S]*?)\nCEREBRUM_HOME_MEMORY_END -->/)
...
const buildCerebrumStateMemoryContext = ({ memory, question, maxStates = 80, maxChars = 12000, now = Date.now() } = {}) => {
  const target = normalizeCerebrumHomeMemory(memory)
...
const classifyCerebrumOpenState = ({ semantic, dpt, payload, valueOptions } = {}) => {
...
const isCerebrumQuietTime = ({ date = new Date(), start = '23:00', end = '07:00' } = {}) => {
...
const buildCerebrumProactiveFallback = ({ language, label, durationMinutes } = {}) => {
  const lang = normalizeHomeLanguage(language)
  const safeLabel = clampText(label || 'home object', 240)
