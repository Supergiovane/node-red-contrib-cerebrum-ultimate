...
const encodeCerebrumCompactValue = (value, depth = 0, seen = new Set()) => {
  ...
  seen.add(value)
  if (Array.isArray(value)) {
    const encoded = value.slice(0, 40).map(item => encodeCerebrumCompactValue(item, depth + 1, seen))
    seen.delete(value)
    return `a(${encoded.join(',')})`
  }
  const encoded = Object.keys(value).slice(0, 80).map(key => (
    `${encodeCompactText(key)}=${encodeCerebrumCompactValue(value[key], depth + 1, seen)}`
  ))
  seen.delete(value)
  return `o(${encoded.join(';')})`
}

const decodeCerebrumCompactValue = value => {
...
const escapeCerebrumCompactField = value => String(value === undefined || value === null ? '' : value)
  .replace(/\\/g, '\\\\')
  .replace(/\t/g, '\\t')
  .replace(/\r/g, '\\r')
  .replace(/\n/g, '\\n')

const unescapeCerebrumCompactField = value => String(value || '').replace(/\\([\\tnr])/g, (match, escaped) => {
  if (escaped === 't') return '\t'
...
const serializeCerebrumCompactHistoryRecord = (event, kind = 'adapter') => {
  ...
  const values = normalizedKind === 'knx'
    ? [
        ...
        event.dpt,
        event.devicename,
        encodeCerebrumCompactValue(sanitizeHistoryValue(event.payload)),
    ...
    : [
        ...
        event.scopeId,
        event.scopeName,
        encodeCerebrumCompactValue(Array.isArray(event.objectTypes) ? event.objectTypes : []),
        encodeCerebrumCompactValue(sanitizeHistoryValue(event.details || {}))
      ]
  return values.map(escapeCerebrumCompactField).join('\t')
}

const parseCerebrumCompactHistoryRecord = (line, kind = 'adapter') => {
  const source = String(line || '')
  if (!source.trim()) return null
  const normalizedKind = kind === 'knx' ? 'knx' : 'adapter'
  const values = source.split('\t').map(unescapeCerebrumCompactField)
  const columns = CEREBRUM_COMPACT_ARCHIVE_COLUMNS[normalizedKind]
  ...
  if (normalizedKind === 'knx') {
    return {
      ...
      dpt: values[4],
      devicename: values[5],
      payload: decodeCerebrumCompactValue(values[6]),
  ...
  const objectTypes = decodeCerebrumCompactValue(values[15])
  const details = decodeCerebrumCompactValue(values[16])
  return {
    ts,
...
const formatCerebrumCompactContextForPrompt = value => {
  const lines = []
...
const formatCerebrumHistorySummaryForPrompt = summary => {
...
const normalizeCerebrumAdapterHistoryEvent = ({ event, adapter, provider, nowTs = Date.now() } = {}) => {
  ...
  return {
    ...
    details: detailsText.length <= CEREBRUM_HISTORY_DETAILS_MAX_CHARS ? details : { truncated: true }
  }
}
...
const buildCerebrumHistoryEventKey = (event, kind = 'adapter') => {
  if (!event || typeof event !== 'object') return ''
  if (kind === 'knx') {
    let payload = ''
...
const createCerebrumHistoryAccumulator = ({ kind = 'adapter', question = '', limit = 120 } = {}) => {
  const normalizedKind = kind === 'knx' ? 'knx' : 'adapter'
...
const formatCerebrumAdapterHistoryEventForPrompt = event => {
  if (!event || typeof event !== 'object') return ''
...
module.exports = {
  ...
  CEREBRUM_HISTORY_DETAILS_MAX_CHARS,
  buildCerebrumHistoryEventKey,
  createCerebrumHistoryAccumulator,
  decodeCerebrumCompactValue,
  encodeCerebrumCompactValue,
