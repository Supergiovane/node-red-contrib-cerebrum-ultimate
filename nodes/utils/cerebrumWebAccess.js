...
const DEFAULT_USER_AGENT = 'Mozilla/5.0 (compatible; Cerebrum-Ultimate/1.0; +https://github.com/Supergiovane/node-red-contrib-cerebrum-ultimate)'

class CerebrumWebAccessError extends Error {
  constructor (code, message) {
    super(message)
    this.name = 'CerebrumWebAccessError'
    this.code = code
  }
}
...
const assertSafeHttpsUrl = (value) => {
  ...
  if (!raw || raw.length > MAX_URL_CHARS) throw new CerebrumWebAccessError('INVALID_URL', 'A valid HTTPS URL is required')
  let parsed
  try { parsed = new URL(raw) } catch (error) { throw new CerebrumWebAccessError('INVALID_URL', 'A valid HTTPS URL is required') }
  if (parsed.protocol !== 'https:') throw new CerebrumWebAccessError('UNSAFE_PROTOCOL', 'Only HTTPS web addresses are allowed')
  if (parsed.username || parsed.password) throw new CerebrumWebAccessError('URL_CREDENTIALS', 'Web addresses containing credentials are not allowed')
  if (parsed.port && parsed.port !== '443') throw new CerebrumWebAccessError('UNSAFE_PORT', 'Only the standard HTTPS port is allowed')
  const hostname = normalizeHostname(parsed.hostname)
  if (!hostname || hostname.length > 253 || hostname.includes('%')) throw new CerebrumWebAccessError('INVALID_HOST', 'The web address has an invalid host')
  if (net.isIP(hostname)) {
    if (!isPublicIpAddress(hostname)) throw new CerebrumWebAccessError('NON_PUBLIC_TARGET', 'Private, local, and reserved web addresses are not allowed')
  } else {
    ...
    if (!hostname.includes('.') || localSuffixes.some(suffix => hostname === suffix || hostname.endsWith(`.${suffix}`))) {
      throw new CerebrumWebAccessError('NON_PUBLIC_TARGET', 'Private, local, and reserved web addresses are not allowed')
    }
  }
  parsed.hash = ''
  return parsed
...
const normalizeDnsRecords = (value) => {
  ...
  for (const item of source) {
    ...
    if (!family) throw new CerebrumWebAccessError('DNS_INVALID', 'The web address returned an invalid DNS response')
    const key = `${family}:${normalized}`
    if (!seen.has(key)) records.push({ address: normalized, family })
    seen.add(key)
  }
  if (!records.length || records.length > 32) throw new CerebrumWebAccessError('DNS_INVALID', 'The web address returned an invalid DNS response')
  return records
}
  ...
  try {
    return await new Promise((resolve, reject) => {
      ...
      timer = setTimeout(() => {
        ...
        settled = true
        controller.abort()
        reject(new CerebrumWebAccessError('TIMEOUT', message))
      }, timeoutMs)
      Promise.resolve()
...
const resolvePublicTarget = async (parsedUrl, { dnsLookup, timeoutMs }) => {
  ...
  try {
    ...
    if (error instanceof CerebrumWebAccessError) throw error
    throw new CerebrumWebAccessError('DNS_FAILED', 'The web address could not be resolved')
  }
  const records = normalizeDnsRecords(rawRecords)
  if (records.some(record => !isPublicIpAddress(record.address))) {
    throw new CerebrumWebAccessError('NON_PUBLIC_TARGET', 'The web address resolves to a private, local, or reserved network')
  }
...
const defaultPinnedHttpsTransport = ({ url, resolvedAddress, headers, timeoutMs, maxBytes, signal }) => new Promise((resolve, reject) => {
  ...
  const abort = () => {
    request.destroy(new CerebrumWebAccessError('TIMEOUT', 'Web request timed out'))
  }
  ...
  const request = https.request(url, {
    ...
    if (Number.isFinite(declaredLength) && declaredLength > maxBytes) {
      response.destroy()
      return finish(new CerebrumWebAccessError('RESPONSE_TOO_LARGE', 'The web response exceeds the size limit'))
    }
    const chunks = []
    let total = 0
    response.on('data', chunk => {
      ...
      if (total > maxBytes) {
        response.destroy()
        finish(new CerebrumWebAccessError('RESPONSE_TOO_LARGE', 'The web response exceeds the size limit'))
        return
      }
      chunks.push(buffer)
    })
...
const responseBodyToBuffer = (body) => {
  ...
  throw new CerebrumWebAccessError('INVALID_RESPONSE', 'The web server returned an invalid response')
}
...
const requestTextWithRedirects = async (initialUrl, options) => {
  ...
  const remainingTimeout = () => {
    ...
    if (remaining <= 0) throw new CerebrumWebAccessError('TIMEOUT', 'Web request timed out')
    return remaining
  }
  for (let redirects = 0; redirects <= options.maxRedirects; redirects++) {
    ...
    try {
      ...
      if (error instanceof CerebrumWebAccessError) throw error
      throw new CerebrumWebAccessError('REQUEST_FAILED', 'The web request failed')
    }

    const statusCode = Number(response && (response.statusCode ?? response.status)) || 0
    if (statusCode >= 300 && statusCode < 400) {
      const location = getHeader(response.headers, 'location')
      if (!location) throw new CerebrumWebAccessError('INVALID_REDIRECT', 'The web server returned an invalid redirect')
      if (redirects >= options.maxRedirects) throw new CerebrumWebAccessError('TOO_MANY_REDIRECTS', 'The web request exceeded the redirect limit')
      let redirected
      try { redirected = new URL(location, current) } catch (error) { throw new CerebrumWebAccessError('INVALID_REDIRECT', 'The web server returned an invalid redirect') }
      current = assertSafeHttpsUrl(redirected.toString())
      continue
    }
    if (statusCode < 200 || statusCode >= 300) throw new CerebrumWebAccessError('HTTP_ERROR', `The web server returned HTTP ${statusCode || 'error'}`)
    const contentEncoding = getHeader(response.headers, 'content-encoding').trim().toLowerCase()
    if (contentEncoding && contentEncoding !== 'identity') throw new CerebrumWebAccessError('UNSUPPORTED_ENCODING', 'The web server returned an unsupported content encoding')
    const contentType = getHeader(response.headers, 'content-type')
    if (!isTextualMimeType(contentType)) throw new CerebrumWebAccessError('UNSUPPORTED_MIME', 'Only textual web content can be opened')
    ...
    if (body.length > options.maxBytes || (Number.isFinite(declaredLength) && declaredLength > options.maxBytes)) {
      throw new CerebrumWebAccessError('RESPONSE_TOO_LARGE', 'The web response exceeds the size limit')
    }
    return {
  ...
  throw new CerebrumWebAccessError('TOO_MANY_REDIRECTS', 'The web request exceeded the redirect limit')
}
 ...
 */
const normalizeCerebrumWebActions = (value, { maxActions = DEFAULT_MAX_ACTIONS } = {}) => {
  let source = value
 ...
 */
const createCerebrumWebAccess = (options = {}) => {
  ...
  const search = async (query, maxResults = DEFAULT_MAX_RESULTS) => {
    ...
    if (!normalizedQuery) throw new CerebrumWebAccessError('INVALID_QUERY', 'A search query is required')
    let lastError = null
    for (const endpoint of searchEndpoints) {
      ...
      try {
        ...
        lastError = new CerebrumWebAccessError('NO_RESULTS', 'The web search returned no usable results')
      } catch (error) {
        lastError = error
      }
    }
    throw lastError || new CerebrumWebAccessError('SEARCH_FAILED', 'The web search failed')
  }
  ...
  const executeAction = async (action) => {
    ...
    try {
      ...
      throw new CerebrumWebAccessError('UNSUPPORTED_OPERATION', 'Unsupported web operation')
    } catch (error) {
      const message = error instanceof CerebrumWebAccessError ? error.message : 'The web operation failed'
      return {
 ...
 */
const executeCerebrumWebActions = async (actions, options = {}) => {
  const normalized = normalizeCerebrumWebActions(actions, { maxActions: options.maxActions })
  const access = createCerebrumWebAccess(options)
  const results = []
