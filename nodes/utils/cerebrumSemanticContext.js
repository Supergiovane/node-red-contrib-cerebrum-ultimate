const CEREBRUM_SEMANTIC_CONTEXT_VERSION = 2
...
const canonicalizeCerebrumCatalog = (catalog) => {
  const byGa = new Map()
...
const serializeRows = ({ section, schema, rows }) => [
  `${section}/${CEREBRUM_SEMANTIC_CONTEXT_VERSION}|${schema.join(',')}`,
...
const serializeCerebrumCloudCatalog = (catalog) => serializeCloudFromCanonical(canonicalizeCerebrumCatalog(catalog))
...
const buildCerebrumSemanticContextStats = ({ source, canonical, cloudText, manifestText }) => {
  ...
  return {
    formatVersion: CEREBRUM_SEMANTIC_CONTEXT_VERSION,
...
const pickOverflowDeclaration = ({ byteBudget, manifestBytes, recordCount, includedCount = 0 }) => {
  const candidates = [
    `KNX-MANIFEST/${CEREBRUM_SEMANTIC_CONTEXT_VERSION}|PARTIAL|included=${includedCount}|records=${recordCount}|bytes=${manifestBytes}|budget=${Number.isFinite(byteBudget) ? byteBudget : 'unlimited'}`,
    `KNX-MANIFEST/${CEREBRUM_SEMANTIC_CONTEXT_VERSION}|PARTIAL|${includedCount}/${recordCount}`,
    `KNX-MANIFEST/${CEREBRUM_SEMANTIC_CONTEXT_VERSION}|OVERFLOW`,
    'OVERFLOW',
    '!'
  ]
...
const packCerebrumSemanticContext = ({ catalog, byteBudget, detailReferences } = {}) => {
  const source = Array.isArray(catalog) ? catalog : []
  const canonical = canonicalizeCerebrumCatalog(source)
  ...
  const packDetails = (maxBytes, records) => {
    ...
    const included = []
    if (records.length > 0) {
      const detailHeader = `KNX-DETAILS/${CEREBRUM_SEMANTIC_CONTEXT_VERSION}|${CLOUD_SCHEMA.join(',')}`
  ...
  const packManifest = (maxBytes, records = canonical) => {
    ...
    const buffer = createAtomicBuffer(maxBytes)
    const header = `KNX-MANIFEST/${CEREBRUM_SEMANTIC_CONTEXT_VERSION}|${MANIFEST_SCHEMA.join(',')}`
  ...
  return {
    ...
    includedDetailIds,
    includedDetailGAs,
    stats: Object.assign({}, buildCerebrumSemanticContextStats({ source, canonical, cloudText, manifestText }), {
...
module.exports = {
  CEREBRUM_SEMANTIC_CONTEXT_VERSION,
  canonicalizeCerebrumCatalog,
