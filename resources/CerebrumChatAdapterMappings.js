; (function (root) {
  const CerebrumChatAdapterMappings = [
    {
      id: 'windkh-telegrambot',
      title: 'windkh/node-red-contrib-telegrambot',
      inputCode: `// Receiver/Event -> Cerebrum
...
if (telegram.type === 'voice') {
    ...
    msg.topic = 'ask';
    msg.cerebrum = Object.assign({}, msg.cerebrum, {
...
msg.prompt = content;
return msg;`,
      outputCode: `// Cerebrum chat output -> Telegram sender
...
const image = msg.cerebrum && msg.cerebrum.image;
...
const audio = msg.cerebrum && msg.cerebrum.audio;
if (audio && Buffer.isBuffer(audio.data)) {
    const voiceLanguage = String(msg.cerebrum.language || (source && source.language) || '').trim().toLowerCase().split(/[-_]/)[0];
    ...
    const confirmation = msg.cerebrum && msg.cerebrum.confirmationRequest;
    ...
    } else if (msg.cerebrum && Array.isArray(msg.cerebrum.suggestions) && msg.cerebrum.suggestions.length) {
        voiceOptions.reply_markup = JSON.stringify({
            keyboard: msg.cerebrum.suggestions.slice(0, 3).map(function (suggestion) {
                return [{ text: Array.from(String(suggestion.text || suggestion.label || '').trim()).slice(0, 64).join('') }];
            }).filter(function (row) { return row[0].text; }),
            resize_keyboard: true,
            one_time_keyboard: true
        });
    } else if (/^knx_confirmation_/.test(String(msg.cerebrum.type || '')) || /^knx_routine_/.test(String(msg.cerebrum.type || ''))) {
    ...
    msg.payload = {
        ...
        fileOptions: {
            filename: audio.filename || 'cerebrum-reply.ogg',
...
} else if (msg.cerebrum && (/^knx_confirmation_/.test(String(msg.cerebrum.type || '')) || /^knx_routine_/.test(String(msg.cerebrum.type || '')))) {
    telegramPayload.options = {
...
return msg;`
    },
    {
      ...
      inputCode: `// RedBot Telegram Receiver -> Cerebrum
...
msg.prompt = content;
return msg;`,
      outputCode: `// Cerebrum chat output -> RedBot Telegram Sender
...
return msg;`
  ...
  if (root) {
    root.CerebrumChatAdapterMappings = CerebrumChatAdapterMappings
  }
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = CerebrumChatAdapterMappings
  }
